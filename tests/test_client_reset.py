...
def test_reset_clears_session(tmp_path):
    """Error reset should clear session_id."""
    state_dir = tmp_path / "state"
    _prepare_state_dir(state_dir)
    config = _make_config(state_dir)
    state = vmain.init_state(config=config)
    ...
    async def test_fn():
        ...
        # Simulate error reset: clear session, set context
        state.session_id = None
        state.pending_context = "[System: Error occurred]"
        state.client = None

        assert state.session_id is None
        assert state.pending_context is not None

        # New session picks up context
        state.pending_context = None
        options = build_client_options(config, state)
        async with ClaudeSDKClient(options=options) as client:
            state.client = client
            ...
            assert responses, "New client should work after reset"

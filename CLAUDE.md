...
**Notification flow**: External systems write JSON files to `~/agent/notifications/` inside the container. `monitor_loop` in `core/loops.py` watches with `watchfiles.awatch()`. Each tick `notification_interrupt_policy.py` decides interrupt vs. pool by precedence: an ordered first-match ruleset (matching `source`/`type`/`sender`/regex-`keyword`), then a per-`(source, type)` user default override, then the notification's static `interrupt` flag as fallback. Interrupting notifications preempt the current turn via `client.interrupt()`; pooled ones wait and are worked through in an idle triage pass. Rules + overrides live in `~/agent/data/notification_policy.json`, applied live each tick (no restart) and edited by the user (`PUT /config/notification-policy` with `{rules?, defaults?}` — each section written independently) or the agent (the `notifications` skill); `source=core` is exempt from rules. Internal boot-time control-flow (greeting, first-start, migrations, skill-sync, config issues) is **not** a notification — it is delivered as non-interruptible **boot turns** enqueued at startup (`main.py` `collect_boot_turns`), processed in order before the agent takes other work.

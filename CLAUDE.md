...
- **Client Core** (`apps/core/`): `@vesta/core`, the shared TypeScript controller every view consumes. Owns the client side of the sync protocol (wire types locked by fixture round-trips, the replica store + reducer, the reconnecting `/sync` socket and the replay-free chat socket, the http client, intents with client ids), the shared chat/roster models, pacing, and notification-content parsing, with a separate `@vesta/core/react` hooks entry so the root stays framework-free. Web imports it as a workspace package; mobile via a `file:` dependency plus Metro `watchFolders`.
...
- **Desktop App** (`apps/desktop/`): Electron wrapper around `@vesta/web` for macOS/Windows/Linux; a TypeScript main process (`src/`: window chrome, preload bridge, connection store, PKCE loopback, latest-release self-updater), no frontend code of its own. Like the mobile app, the desktop app is a drifting client of vestad: it opens `/sync` and lets the tolerant-floor protocol handshake decide compatibility, and it self-updates to the latest published release on its own (checked on launch, downloaded in the background, applied on the next relaunch), independent of the gateway's version. It may drift *behind* the gateway freely (the protocol floor keeps an older app talking to a newer gateway), but it must never run *ahead* of it: when the client's release version is newer than the gateway's, the socket raises a recoverable `gateway_behind` state and `GatewayBehindScreen` blocks with an "update gateway" action, self-healing once the gateway restarts newer (the release-version compare and both blocked states live in `@vesta/core`, shared by web/desktop/mobile). When the app and gateway support non-overlapping protocol floors the socket goes terminally `incompatible` and `IncompatibleScreen` takes over, telling the user to update one side. Updating the gateway is a separate, deliberate action (the UpdatePill).
...
**Client protocol (three planes)**: clients talk to vestad only; vestad is the single client-facing contract producer. The **state plane** is one multiplexed WebSocket, `GET /sync`: `hello {version, protocol, floor}` (tolerant-floor compat, unknown frames ignored by rule), an immediate tail-less snapshot (gateway + agent roster incl. mid-build agents + pending notifications), in-band `reauth`, and typed deltas (`state`, `agent`, `agent_removed`, `notifications`, `user_notification`). It is a read-only fleet-state projection: no `watch`/`unwatch`, no per-agent live edge, no chat. The `user_notification` delta carries `{agent, kind, title, body}` (`kind` in `message`|`rate_limited`) and is the one ephemeral delta (no snapshot backlog). The **command plane** is addressed HTTP intents carrying a client `intent_id`; the chat send intent POSTs to the app-chat service through the proxy (`POST /agents/{name}/app-chat/message`), the skill dedups repeated `intent_id`s and persists + emits before the 200 (so the 200 is durable delivery), and delivery truth is the echo carrying that id on the chat socket, never merely the 200; a 502/503/504/timeout is retryable (the service is unreachable). The **service/bulk plane** (the authenticated per-agent proxy) carries what a state tree cannot: the whole app-chat service (that intake endpoint, paged id-cursored history `GET .../app-chat/history`, and the replay-free live chat socket `GET .../app-chat/ws`), SSE logs, voice STT WS, backups. Vestad's aggregator (`vestad/src/sync/`) feeds the state plane from one read-only internal tap per running agent (`agent_status.rs`), shrunk to activity + pending notifications + timezone. User-facing notifications are produced separately: an agent-side user-notification primitive (the vestad `user-notification` script the app-chat daemon shells on a reply, or core on a rate limit) loops back to vestad (`POST /agents/{me}/user-notification`, agent-token, self-scoped), which fans the `user_notification` delta to connected `/sync` clients and, for a reply, an Expo push to backgrounded mobile; vestad keeps producing its own agent-status pushes. The agent's own WS event bus is internal (vestad's tap and in-container skills consume it, never client-exposed) and carries only agent-internal events plus the tap-read subset. Chat tails come from paged history joined to the chat socket's replay-free live echo by event id; a reconnect gap self-heals by refetching the tail by id (reseed).

**Message flow**: a client sends chat by POSTing to the app-chat service through the proxy (`POST /agents/{name}/app-chat/message`); the skill service dedups repeated `intent_id`s (retry idempotency), persists the user message to its own store (sqlite + FTS5), streams the echo on its replay-free live chat socket (`GET .../app-chat/ws`), and in the same coroutine writes a `source=app-chat` notification file, so all app chat reaches the model through the notification flow below. Persist + emit complete before the 200, so a message the UI already echoed as delivered is durable even with no socket connected (the client reseeds the tail by id on reconnect). The agent replies with `app-chat send`: the daemon persists the reply, fans it to any connected chat sockets, and shells the vestad `user-notification` script so a backgrounded client gets a user notification + push. Core does not transport chat: the agent's own bus carries only agent-internal events plus the tap-read subset. The processor's queue (`message_processor` in `core/loops.py`) has exactly two producers: notification batches (`process_batch`) and boot turns (`main.py collect_boot_turns`). Turns drive the SDK client (`client.query()`) and stream response blocks (text, thinking, tool use) back through the EventBus to all WS subscribers in real time. **Preemption**: the processor's queue-watcher owns preempt delivery: an item landing mid-turn is delivered via `send_preempt` (`core/client.py`) as a `priority:"now"` user message, and the CLI ends the running turn at its next step boundary (a foreground tool call in flight finishes first; the latched abort applies when it returns), with background subagents surviving (issue #982). Delivery is completion: the CLI persists the message into the session on receipt but guarantees no per-prompt result (it merges rapid queued prompts into one turn, starts turns of its own such as background task notifications, and its ResultMessages carry no prompt identity), so the watcher clears a delivered item's notification files on the spot and never opens a Vesta turn for it; the output streams through the session's single consumer, which also keeps the activity state honest for turnless work (`_dispatch_message`). Undeliverable items (idle gap, boot turn, compaction, unauthenticated, failed write) queue as ordinary turns with plain 1:1 query/result pairing. The SDK `interrupt()` control request fires only on failure paths (silence/query timeout, auth loss): in headless mode the CLI's handler kills every backgrounded subagent, which is why routine preemption never uses it.
...
apps/mobile/  # Expo/React Native app (@vesta/mobile, own lockfile)
...
- **Test observable behavior through public surfaces.** Emit an `AssistantEvent` and assert the resulting event stream / persisted state, not which internal function fired. Behavior names read as sentences.

...
- **Skills** (`agent/skills/` + `agent/core/skills/`): each skill directory has `SKILL.md` + scripts. `agent/core/skills/` holds built-ins shipped with the agent (e.g. `app-chat`, `upstream-sync`); `agent/skills/` holds the rest. Every skill lives on disk (the box is a full checkout); installing only activates one. `data/installed-skills.txt` lists the active optional skills, and the boot entrypoint's `link-skills.sh` rebuilds `~/.claude/skills` (the symlink farm Claude Code discovers skills from) each boot: core skills always, optional skills only when listed. `skills-install` appends a name + relinks, `skills-remove` drops it; instant, offline, no git. Skills are plain directories, not MCP servers; the only MCP server is the agent's own native tool registry (`core/tools.py`), exposed in-process via the SDK's `create_sdk_mcp_server`.
...
**Skills**: each skill in `agent/skills/{name}/` or `agent/core/skills/{name}/` has `SKILL.md` (YAML frontmatter with name/description) + CLI tools, loaded natively by Claude Code (via `setting_sources` + the `~/.claude/skills` symlink farm). Every skill is on disk (the box is a full checkout); installing on a box means listing it in `data/installed-skills.txt` so `link-skills.sh` symlinks it into `~/.claude/skills` (`skills-install`), removal drops the line. `skills-search` searches the local skills dir. The bracketed `[Fill in...]` placeholders in a skill's `SKILL.md` are intentional personalization scaffolding the agent fills in per user over time; leave them intact, they are not dead space.
...
A new skill needs no index step: skills are discovered from disk. If it should be installed on every box by default, add its name to `agent/core/default-skills.txt`.
...
- **Beyonce rule, mapped to suites.** Crash recovery / resume -> `test_crash_recovery.py`; priority-now preemption -> `test_preempt.py`; failure-path interrupts / compaction -> `test_interrupts.py`; notification batching -> `test_notifications.py`; backup/restore and ports -> vestad tests. If you do not want it to break, it has a test.

...
- **Skills** (`agent/skills/` + `agent/core/skills/`): each skill directory has `SKILL.md` + scripts. `agent/core/skills/` holds built-ins shipped with the agent (e.g. `app-chat`, `upstream-sync`); `agent/skills/` holds the rest. Every skill lives on disk (the box is a full checkout); a skill is active or inactive, there is no install step. `data/active-skills.txt` lists the active optional skills, and the boot entrypoint's `link-skills.sh` rebuilds `~/.claude/skills` (the symlink farm Claude Code discovers skills from) each boot: core skills always, optional skills only when listed. `skills-activate` appends a name + relinks, `skills-deactivate` drops it; instant, offline, no git. Skills are plain directories, not MCP servers; the only MCP server is the agent's own native tool registry (`core/tools.py`), exposed in-process via the SDK's `create_sdk_mcp_server`.
...
**Skills**: each skill in `agent/skills/{name}/` or `agent/core/skills/{name}/` has `SKILL.md` (YAML frontmatter with name/description) + CLI tools, loaded natively by Claude Code (via `setting_sources` + the `~/.claude/skills` symlink farm). Every skill is on disk (the box is a full checkout); activating on a box means listing it in `data/active-skills.txt` so `link-skills.sh` symlinks it into `~/.claude/skills` (`skills-activate`), deactivating drops the line. `skills-search` searches the local skills dir. The bracketed `[Fill in...]` placeholders in a skill's `SKILL.md` are intentional personalization scaffolding the agent fills in per user over time; leave them intact, they are not dead space.

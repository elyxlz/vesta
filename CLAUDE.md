...
- **Client Core** (`apps/core/`): `@vesta/core`, the shared TypeScript controller every view consumes. Owns the client side of the sync protocol (wire types locked by fixture round-trips, the replica store + reducer, the reconnecting socket, the http client, intents with client ids), the shared chat/roster models, pacing, and notification-content parsing, with a separate `@vesta/core/react` hooks entry so the root stays framework-free. Web imports it as a workspace package; mobile via a `file:` dependency plus Metro `watchFolders`.

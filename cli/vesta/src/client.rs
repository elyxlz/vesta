...
impl Client {
    ...
    /// Connect to WebSocket and run interactive chat.
    pub fn chat(&self, name: &str) -> Result<(), String> {
        let ws_url = self
            .base_url
            .replace("https://", "wss://")
            .replace("http://", "ws://");
        let url = format!("{}/agents/{}/ws?token={}", ws_url, name, self.api_key);

        // Parse host:port from WS URL and manually connect with TLS
        let parsed: url::Url =
        ...
        loop {
            // Check for stdin input (non-blocking via channel)

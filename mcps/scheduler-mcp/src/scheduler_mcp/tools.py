...
def set_reminder(
    ...
    if not any([datetime, seconds, minutes, hours, days, recurring, interval_minutes]):
        raise ValueError("Must provide either datetime, seconds, minutes, hours, days, recurring, or interval_minutes")
    ...
    if recurring == "daily" or (datetime and ":" in datetime and "T" not in datetime):
        ...
        if day_of_week and time_of_day:
            ...
            if day_of_week.lower() not in day_map:
                raise ValueError(f"Invalid day_of_week: {day_of_week}. Use monday, tuesday, etc.")

            if ":" not in time_of_day:
                raise ValueError("time_of_day must be in HH:MM format")

            hour, minute = time_of_day.split(":")
            trigger = CronTrigger(day_of_week=day_map[day_of_week.lower()], hour=int(hour), minute=int(minute))
    ...
    scheduler.add_job(write_notification, trigger, args=[reminder_id, message], id=reminder_id)

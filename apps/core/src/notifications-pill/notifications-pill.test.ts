...
describe("pillVisibleWhileViewing", () => {
  it("hides a message from the agent whose page is open", () => {
    expect(pillVisibleWhileViewing(item(0, "message", "aria"), "aria")).toBe(
      false,
    );
  });

  it("shows a message from any other agent", () => {
    expect(pillVisibleWhileViewing(item(0, "message", "aria"), "apollo")).toBe(
      true,
    );
  });
...
describe("pillDisplayLine", () => {
  it("joins a title and body as title: body", () => {
    expect(pillDisplayLine({ title: "aria", body: "hi there" })).toBe(
      "aria: hi there",
    );
  });

  it("renders a bodiless notification as the title alone", () => {
    expect(pillDisplayLine({ title: "aria stopped", body: "" })).toBe(
      "aria stopped",
    );
  });
});

...
function run(
  actions: FeedAction[],
  from: NotificationFeed = EMPTY_FEED,
): NotificationFeed {
...
describe("reduceFeed: pages", () => {
  ...
  it("marks a failed older load on its own slot, and a retry clears it", () => {
    ...
    expect(run([{ type: "page_loading", before: 4 }], failed).older).toBe(
      "loading",
    );
  });
});
...
describe("feedNeedsMarkSeen", () => {
  it("asks for a catch-up when the gateway's newest stamp or a loaded row is past the held watermark", () => {
    ...
    expect(
      feedNeedsMarkSeen(
        run([{ type: "arrived", readInPlace: false, entry: entry(2) }], held),
        150,
      ),
    ).toBe(true);
  });
  ...
  it("never asks before a session ever opened", () => {
    expect(
      feedNeedsMarkSeen(
        run([{ type: "arrived", readInPlace: false, entry: entry(2) }]),
        500,
      ),
    ).toBe(false);
  });
...
describe("feedUnseen", () => {
  ...
  function loaded(actions: FeedAction[]): NotificationFeed {
    return run([
      { type: "page_loaded", page: [], pageSize: PAGE_SIZE },
      ...actions,
    ]);
  }
  ...
  it("does not raise the dot for a notification the user read in the chat", () => {
    const feed = loaded([
      { type: "arrived", readInPlace: true, entry: entry(3, 300) },
    ]);
...
describe("feedSections", () => {
  it("splits on the held watermark, and stays unsectioned before the first open or when never caught up", () => {
    ...
    expect(
      feedSections(reduceFeed(loaded, { type: "open", seenAt: 0 })),
    ).toBeNull();
    const sections = feedSections(
      reduceFeed(loaded, { type: "open", seenAt: 100 }),
    );
...
describe("feedView", () => {
  it("reads loading until the first page answers, then failed, empty, or rows", () => {
    ...
    expect(
      feedView(run([{ type: "page_loaded", page: [], pageSize: PAGE_SIZE }])),
    ).toBe("empty");
    expect(
      feedView(run([{ type: "arrived", readInPlace: false, entry: entry(1) }])),
    ).toBe("rows");
    expect(
      feedView(
        run([
          { type: "arrived", readInPlace: false, entry: entry(1) },
          { type: "page_failed" },
        ]),
      ),
    ).toBe("rows");

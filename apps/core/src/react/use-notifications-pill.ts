...
export function useNotificationsPill(
  ...
  useEffect(() => {
    ...
    return source.subscribeDeltas((delta: Delta) => {
      ...
      if (!item || paused || !pillVisibleWhileViewing(item, viewedAgent))
        return;
      const orbState = orbStateFor(item.agent);
      setQueue((current) =>
        enqueuePillNotification(current, { ...item, orbState }),
      );
    });
  }, [source]);
  ...
  const current =
    head && pillVisibleWhileViewing(head, options.viewedAgent) ? head : null;

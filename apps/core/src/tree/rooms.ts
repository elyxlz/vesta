...
// What a room is, read off its own shape: one agent and no name is the direct chat on that agent's
// page, two agents and no name is a peer room, and anything named (or larger) is a group.
export type RoomKind = "direct" | "peer" | "group";

export function roomKind(room: Room): RoomKind {
  if (room.name !== null) return "group";
  if (room.agents.length === 1) return "direct";
...
// the order is stable across snapshots.
export function selectRooms(tree: Tree | null): Room[] {
  return [...(tree?.rooms ?? [])].sort((left, right) => {

...
describe("isKeyFresh", () => {
  ...
  it("re-mints before expiry rather than at it", () => {
    expect(
      isKeyFresh({ key: "k", expiresAt: NOW_SECS + 43_200 }, NOW_SECS),
    ).toBe(true);
    expect(isKeyFresh({ key: "k", expiresAt: NOW_SECS + 60 }, NOW_SECS)).toBe(
      false,
    );
    expect(isKeyFresh({ key: "k", expiresAt: NOW_SECS - 1 }, NOW_SECS)).toBe(
      false,
    );
  });
});
...
describe("createServiceKeyCache", () => {
  it("mints once per agent and service, then reuses the key", async () => {
    ...
    const cache = createServiceKeyCache({
      http,
      gateway: () => "https://gw-a",
    });
  ...
  // Two consumers ask on a cold cache in the same tick: React's double effect in dev asks twice.
  // Without one shared in-flight mint each would mint, and the later one would overwrite the
  // other's cached key.
  ...
  it("mints again after a failed mint rather than holding on to it", async () => {
    ...
    await expect(cache.get("alpha", "dashboard")).rejects.toThrow(
      "gateway unreachable",
    );
    expect(await cache.get("alpha", "dashboard")).toBe("second");
    expect(calls).toHaveLength(2);
  });

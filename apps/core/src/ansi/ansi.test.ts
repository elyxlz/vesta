...
describe("parseAnsi", () => {
  ...
  it("supports bright, 256-color, and true-color output", () => {
    expect(
      parseAnsi("\x1b[94mblue\x1b[38;5;196mred\x1b[38;2;1;2;3mrgb"),
    ).toEqual([
  ...
  it("carries the terminal attributes and clears them individually", () => {
    expect(
      parseAnsi("\x1b[1;3;4mattrs\x1b[24mno-underline\x1b[22mno-bold"),
    ).toEqual([
...
describe("resolveAnsiColor", () => {
  ...
  it("maps standard and bright indices through the base palette", () => {
    ...
    expect(resolveAnsiColor({ kind: "palette", index: 12 }, palette)).toBe(
      "p12",
    );
  });
  ...
  it("passes truecolor through unchanged", () => {
    expect(
      resolveAnsiColor({ kind: "rgb", value: "rgb(1, 2, 3)" }, palette),
    ).toBe("rgb(1, 2, 3)");
  });

  it("maps the xterm color cube", () => {
    expect(resolveAnsiColor({ kind: "palette", index: 196 }, palette)).toBe(
      "rgb(255, 0, 0)",
    );
  });

  it("maps the grayscale ramp", () => {
    expect(resolveAnsiColor({ kind: "palette", index: 232 }, palette)).toBe(
      "rgb(8, 8, 8)",
    );
    expect(resolveAnsiColor({ kind: "palette", index: 255 }, palette)).toBe(
      "rgb(238, 238, 238)",
    );
  });
});

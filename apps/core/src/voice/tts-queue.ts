// The one TTS playback pipeline both apps drive: an ordered queue over an injected
// player, so consecutive agent replies play back to back instead of cutting each
// other off. Prefetch warms the prepare step during the typing-pacing delay so
// playback starts the streamed GET the moment a message is revealed.

export interface SpeechPlayer {
  // Registers the text with the voice service and resolves the utterance id.
  prepare: (text: string) => Promise<string>
  // Streams one prepared utterance to the speaker. Resolves when playback ends
  // or the signal aborts; rejects only on a real playback failure.
...
export function createTtsQueue(player: SpeechPlayer, callbacks: TtsQueueCallbacks): TtsQueue {
  ...
  // Bumped by stop to invalidate an in-flight drain loop, so a stop-then-speak
  // sequence never leaves two loops draining the queue at once.
  let epoch = 0
  ...
  const drain = async (): Promise<void> => {
    ...
    // A newer epoch (stop) superseded this loop; the new loop owns the shared
    // flags, so exit without resetting them.
  ...
  return {
    ...
    prefetch: (text) => {
      ...
      // Consumed lazily by drain; swallow here so a dropped prefetch never

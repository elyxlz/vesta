// One live transcription session over the voice service's /stt/listen socket, shared by
// every client: the socket lifecycle, the Deepgram TurnInfo protocol, transcript
// accumulation, and the pre-open audio buffer. Platforms inject only how audio is captured
// and how the socket is dialed.
...
export interface AudioCapture {
  ...
  // mono PCM. Rejecting is the permission or hardware failure path.
...
export interface SttSessionOptions {
  // Dictation: committed turns concatenate into one growing transcript that stop() returns,
  // instead of each turn ending on its own.
  accumulate?: boolean;
}

export interface SttSession {
  start: () => Promise<void>;
  // Ends the session and returns the full text committed so far (empty when idle). An
  // in-flight partial turn is dropped, never sent as half a turn.
  stop: () => string;
  active: () => boolean;
}
...
export function createSttSession(
  ...
  // Bumped by stop; a start still in flight compares its own generation and abandons
  // quietly instead of throwing at the caller that asked to stop.
  let generation = 0;
  ...
  const fail = (message: string): void => {
    // Report before cleanup: cleanup fires onActiveChange(false), and a consumer that drops
    // its session reference on going inactive would otherwise miss this error.
  ...
  const onFrame = (frame: ArrayBuffer): void => {
    ...
    while (
      pending.length > 0 &&
      pendingBytes + frame.byteLength > MAX_PENDING_AUDIO_BYTES
    ) {
  ...
  const handleMessage = (data: string): void => {
    ...
    if (event.type === "TurnInfo") {
      ...
      if (event.event === "EndOfTurn") {
        ...
        if (accumulate) {
          ...
          // Clear the display before delivering the turn, so a consumer that renders the
          // transcript into a draft box gets the final text last.
    ...
    if (event.type === "ConfigureFailure")
      fail("Transcription configuration error");
  ...
  const stop = (): string => {
    if (state === "idle") return "";
    generation += 1;
    // Dictation returns the whole display, including a turn still in progress, since the
    // user confirmed what they see. A conversation drops any partial: its turns already
    // sent on their own end, and half a turn is never a message.
    const full = accumulate ? displayText().trim() : "";
    cleanup();
    return full;
  };

...
describe("agentStatusKind", () => {
  it.each<AgentStatusKind>(["alive", "working", "needs-user", "down"])(
    ...
    (kind) => {
      expect(
        EVERY_STATUS.filter((status) => agentStatusKind(status) === kind),
      ).toEqual(KIND_MEMBERS[kind]);
    },
  );

  it("waits on the user only where the user is the one who can act", () => {
    expect(EVERY_STATUS.filter(agentNeedsUser)).toEqual(
      KIND_MEMBERS["needs-user"],
    );
  });
  ...
  it("never calls a status both connectable and down", () => {
    expect(
      EVERY_STATUS.filter((s) => agentIsConnectable(s) && agentIsDown(s)),
    ).toEqual([]);
  });
});
...
describe("agentOrbState", () => {
  ...
  it.each<AgentStatus>(["stopped", "dead", "not_found"])(
    "turns the orb off for %s",
    (status) => {
...
describe("a running operation", () => {
  ...
  it("outranks a status the operation itself caused", () => {
    ...
    expect(agentStatusLabel("stopped", "idle", "backing_up")).toBe(
      "backing up...",
    );
  });
  ...
  it("keeps the orb alive while a restore tears the container down", () => {
    ...
    expect(agentStatusLabel("not_found", "idle", "restoring")).toBe(
      "restoring...",
    );
  });
...
describe("orbIsLive", () => {
  it.each<OrbVisualState>(["alive", "thinking", "busy"])(
    "animates the %s orb",
    (state) => {
  ...
  it.each<OrbVisualState>(["limited", "off", "deleting"])(
    "holds the %s orb still",
    (state) => {
...
describe("a binding rate limit", () => {
  ...
  it("dims an alive agent even while the CLI's retries read as thinking", () => {
    ...
    expect(agentOrbState("alive", "thinking", null, false, window)).toBe(
      "limited",
    );
  });

  it("never outranks an operation, a boot, or a non-alive status", () => {
    expect(agentOrbState("alive", "idle", "backing_up", false, window)).toBe(
      "busy",
    );
    expect(agentOrbState("alive", "idle", null, true, window)).toBe("busy");
    expect(
      agentOrbState("not_authenticated", "idle", null, false, window),
    ).toBe("attention");
  });
  ...
  it("labels the agent with the reset countdown when the window carries one", () => {
    ...
    expect(agentStatusLabel("alive", "idle", null, false, window)).toBe(
      "rate limited",
    );
  });
...
describe("agentStatusLabel", () => {
  it("names the user as the actor for the states only they can resolve", () => {
    expect(agentStatusLabel("not_authenticated", "idle")).toBe(
      "needs you to sign in",
    );
    expect(agentStatusLabel("unprovisioned", "idle")).toBe(
      "needs to be set up",
    );
  });
  ...
  it("renders an alive agent still in its boot turns as one continuous waking up", () => {
    expect(agentStatusLabel("alive", "thinking", null, true)).toBe(
      "waking up...",
    );
  ...
  it("scopes booting to alive: other statuses keep their own words", () => {
    expect(agentStatusLabel("not_authenticated", "idle", null, true)).toBe(
      "needs you to sign in",
    );
    expect(agentStatusLabel("starting", "idle", null, true)).toBe(
      "waking up...",
    );
  });

  it("keeps an operation outranking the boot flag", () => {
    expect(agentStatusLabel("alive", "idle", "backing_up", true)).toBe(
      "backing up...",
    );
  });
});

...
function texts(state: ChatState): string[] {
  return state.messages.map((m) => (m.type === "chat" ? m.text : ""));
}
...
describe("keyed hold store", () => {
  ...
  it("keeps a loadMore-extended history across the epoch", () => {
    ...
    const seed = store.read(key);
    if (!seed) throw new Error("expected held state");
    expect(texts(seed)).toEqual(["a", "b", "c"]);
  ...
  it("keeps each agent's conversation in its own cell across switches", () => {
    ...
    expect(ada ? texts(ada) : null).toEqual(["a"]);
    expect(ben ? texts(ben) : null).toEqual(["b"]);
  });

  it("a connection (gateway) switch never reads another gateway's cell", () => {
    const store = createKeyedHoldStore<ChatState>();
    store.persist(agentHoldKey("ada", GW), tailWith([1, "a"]));
    expect(store.read(agentHoldKey("ada", "https://gw-b"))).toBeNull();
  });
  ...
  it("notifies a subscriber of the written cell only through its own key", () => {
    const store = createKeyedHoldStore<string>();
    const seen: (string | null)[] = [];
    store.store.subscribe((state) => seen.push(state.cells.get("a") ?? null));
    store.persist("a", "draft");
    store.persist("b", "other");
    expect(seen).toEqual(["draft", "draft"]);

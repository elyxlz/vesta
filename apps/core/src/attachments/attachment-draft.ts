import { MAX_ATTACHMENTS_PER_MESSAGE, type ChatAttachment } from "./attachment-model"
...
function update(
  ...
  return drafts.map((draft) => (draft.localId === localId ? change(draft) : draft))
}
...
export function setDraftWaiting(drafts: DraftAttachment[], localId: string): DraftAttachment[] {
...
export function failDraft(
  ...
  return update(drafts, localId, (draft) => ({ ...draft, status: "error", error }))
}

export function removeDraft(drafts: DraftAttachment[], localId: string): DraftAttachment[] {
...
export function draftsReady(drafts: DraftAttachment[]): boolean {
  return drafts.length > 0 && drafts.every((draft) => draft.status === "uploaded")
}

export function uploadedIds(drafts: DraftAttachment[]): string[] {
  return drafts.flatMap((draft) => (draft.attachment ? [draft.attachment.id] : []))
}

export function uploadedAttachments(drafts: DraftAttachment[]): ChatAttachment[] {
  return drafts.flatMap((draft) => (draft.attachment ? [draft.attachment] : []))
}

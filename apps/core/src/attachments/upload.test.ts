...
import { UploadError, uploadAttachment, type Connectivity, type UploadRunState } from "./upload"
  ...
  const http: HttpClient = {
    ...
    json: <T>(path: string, init?: RequestInit) =>
      new Promise<T>((resolve, reject) => {
        const call: Call = { path, init, resolve: resolve as (value: unknown) => void, reject }
...
function start(http: HttpClient, size: number, run = harness()) {
  ...
  const handle = uploadAttachment(http, "apollo", blob, { ...META, size }, run.deps, run.callbacks)
  return { handle, run }
}
...
describe("uploadAttachment", () => {
  it("creates, PUTs sequential offsets, completes, and reports progress", async () => {
    ...
    expect(first.path).toBe("/agents/apollo/app-chat/attachments/att1/data?offset=0")
    ...
    expect(complete.path).toBe("/agents/apollo/app-chat/attachments/att1/complete")
  ...
  it("never shrinks below the floor", async () => {
    ...
    for (let failure = 0; failure < 4; failure += 1) {
      ...
      await settle()
      run.fireNextTimer()
      ;(await next()).resolve({ received: 0, size: MIN_CHUNK_BYTES * 8, finalized: false })
    }
  ...
  it("rejects terminal outcomes without a single retry", async () => {
    ...
    const over = uploadAttachment(
      ...
      "apollo",
      blob,
      { ...META, size: MAX_ATTACHMENT_BYTES + 1 },
      run.deps,
      run.callbacks,
    )
    ...
    await expect(old.handle.result).rejects.toMatchObject({ reason: "unsupported_agent" })
    ...
    await expect(short.handle.result).rejects.toMatchObject({ reason: "failed" })
  })
...
describe("409 recovery under a broken link", () => {
  it("parks with backoff instead of busy-looping when the probe also fails", async () => {
    ...
    expect([...run.timers.values()][0]?.ms).toBe(RETRY_BASE_MS)
    run.fireNextTimer()
    ;(await next()).resolve({ received: 0, size: INITIAL_CHUNK_BYTES, finalized: false })
    const retry = await next()
    expect(retry.path).toContain("offset=0")
    handle.abort()
    await expect(handle.result).rejects.toThrow("aborted")

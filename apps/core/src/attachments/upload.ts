...
export type UploadErrorReason = "too_large" | "unsupported_agent" | "failed" | "aborted"
...
export function uploadAttachment(
  ...
  const sendChunks = async (id: string) => {
    ...
    while (offset < meta.size) {
      checkAborted()
      const chunk = blob.slice(offset, Math.min(offset + chunkBytes, meta.size))
      ...
      try {
        ...
        if (error instanceof ApiError && error.status === 409) {
          // Offset drift, including the lost-response replay: adopt the server's staged size. A
          // failed probe parks below like any retryable failure, never a zero-delay re-PUT loop.
          try {
            offset = await probeOffset(id)
            callbacks.onProgress(Math.min(offset, meta.size), meta.size)
            continue
          } catch (probeError) {
            checkAborted()
            const probeVerdict = classify(probeError)
            if (probeVerdict !== "retryable") throw new UploadError(probeVerdict)
          }
        } else {
          const verdict = classify(error)
          if (verdict !== "retryable") throw new UploadError(verdict)
        ...
        try {
          ...
          // Keep the current guess; the next PUT's 409 re-enters recovery.
        }
      }
    }
  }
  ...
  const complete = async (id: string): Promise<ChatAttachment> => {
    for (let attempt = 0; ; attempt += 1) {
      checkAborted()
      try {
        const done = await jsonPost<CompleteResponse>(`${base}/${id}/complete`, {})
        return done.attachment
      } catch (error) {
        checkAborted()
        // A complete 409 is a size mismatch: the staged bytes are wrong, no retry can fix them.
        if (error instanceof ApiError && error.status === 409) throw new UploadError("failed")
        const verdict = classify(error)
        if (verdict !== "retryable") throw new UploadError(verdict)
        await park(attempt)
      }

...
const DONE: ChatAttachment = { id: "srv1", name: "photo.jpg", mime: "image/jpeg", size: 1000 }
...
describe("attachment drafts", () => {
  it("adds an uploading draft and enforces the per-message cap", () => {
    ...
    expect(list[0]).toMatchObject({ status: "uploading", progress: 0, name: "photo.jpg" })
  })
  ...
  it("tracks progress, waiting, and finalize", () => {
    ...
    expect(list[0]).toMatchObject({ status: "uploaded", progress: 1, attachment: DONE })
  })

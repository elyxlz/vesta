...
export function AgentLayout() {
  ...
  useEffect(() => {
    if (agent) usePreferences.getState().update({ lastAgent: agent.name });
  }, [agent]);
...
function AgentLayoutInner() {
  ...
  const chatCollapsedAgents = usePreferences((s) => s.chatCollapsed);
  ...
  const chatCollapsed =
    ...
    (choice !== null && choice.name === name
      ? choice.collapsed
      : chatCollapsedAgents.includes(name));

...
fn start_stop_restart() {
    ...
    let s = c.agent_status(&agent.name).unwrap();
...
fn destroy_removes_agent() {
    ...
    let s = c.agent_status(&name).unwrap();
...
fn start_nonexistent_fails() {
    let c = SERVER.client();
    assert!(c.start_agent("does-not-exist").is_err());
}

#[test]
fn stop_nonexistent_fails() {
    let c = SERVER.client();
    assert!(c.stop_agent("does-not-exist").is_err());
}
...
fn empty_name_fails() {
    let c = SERVER.client();
    assert!(c.create_agent("", false).is_err());
}

#[test]
fn special_chars_name_normalized() {
    let c = SERVER.client();
    let result = c.create_agent("!!!", false);
    assert!(result.is_err(), "name normalizing to empty should fail");
}
...
async fn ws_connect_to_running_agent() {
    ...
    // Connection should succeed (or fail with a proxy error if agent isn't ready yet — both are acceptable,
    // the important thing is the TLS handshake and auth succeed)
    match result {
        Ok((ws, _)) => {
            drop(ws);
        }
        Err(e) => {
            let err = e.to_string();
            // 503 = agent WS not ready yet (acceptable), auth/TLS errors are not
            assert!(

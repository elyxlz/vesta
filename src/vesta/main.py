...
from claude_code_sdk.types import McpStdioServerConfig, AssistantMessage, TextBlock, ToolUseBlock

from .memory_agent import preserve_conversation_memory

ephemeral_mode = os.environ.get('EPHEMERAL', '').lower() == 'true'
MCP_SERVERS = {
    "microsoft": {"command": "uv", "args": ["run", "--directory", "mcps/microsoft-mcp", "microsoft-mcp"]},
    "whatsapp": {"command": "uv", "args": ["run", "--directory", "mcps/whatsapp-mcp/whatsapp-mcp-server", "main.py"]},
    "scheduler": {"command": "uv", "args": ["run", "--directory", "mcps/scheduler-mcp", "scheduler-mcp"]}
}
...
C = {
    'dim': '\033[2m', 'cyan': '\033[96m', 'magenta': '\033[95m',
    'yellow': '\033[93m', 'green': '\033[92m', 'reset': '\033[0m'
}
...
def get_mcp_config():
    ...
    return {
        name: McpStdioServerConfig(
            ...
            env={"PYTHONUNBUFFERED": "1", "NOTIFICATIONS_DIR": str(root / "notifications"), "DATA_DIR": str(root / "data")}
...
async def run_vesta():
    ...
    async def monitor():
        ...
        while not SHUTDOWN_EVENT.is_set():
            ...
            if now - last_bridge_check >= timedelta(seconds=30):
                if not subprocess.run(["pgrep", "-f", "whatsapp-bridge"], capture_output=True, text=True).stdout.strip():
    ...
    tasks = [asyncio.create_task(t) for t in [handle_input(), process_queue(), monitor()]]

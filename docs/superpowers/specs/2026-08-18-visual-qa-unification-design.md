...
- Card: one scenario. Slots: `platformsOfFamily(family)` minus its `platforms` exclusions, in `PLATFORMS` order. The card grid is up to three columns and wraps, so a web card is a light row over a dark row and a mobile card is one row, with no theme logic in the view.

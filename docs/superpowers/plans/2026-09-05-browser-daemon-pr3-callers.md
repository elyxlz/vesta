...
- [ ] Load `vesta-prompt-guide`; write `SKILL.md` to the Global Constraints structure (target under 120 lines; the recipes stay as linked references at the end); write `SETUP.md`.
...
- [ ] Spec: in Internal callers → Microsoft, replace "One stealth session per account ... `--stealth`" with the Chromium ruling and its reason (headless Firefox cannot load the Outlook/Teams SPAs; SSO needs cookies, not stealth), and the one-time re-sign-in.

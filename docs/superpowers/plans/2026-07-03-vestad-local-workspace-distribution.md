...
- Known intentional deviations: `ensure_workspace` failure is loud-but-nonfatal at startup (boxes 404 on the bundle; vestad still serves) — the spec's "checked with a clear error" is satisfied by the distinct `GitMissing` message; a hard startup abort would brick otherwise-working installs over a missing git binary.

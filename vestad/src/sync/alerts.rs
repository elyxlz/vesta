...
pub(crate) fn alert_for(event: &Value) -> Option<String> {
    let text = event.get("text")?.as_str()?;
    match event.get("type")?.as_str()? {
        "chat" => Some(truncate_preview(text)),
        "rate_limited" => Some(text.to_string()),
        _ => None,
    }
}
...
mod tests {
    ...
    fn chat_is_worthy_with_a_truncated_preview() {
        ...
        let preview = alert_for(&serde_json::json!({ "type": "chat", "text": long })).expect("chat alert");
        assert_eq!(preview.chars().count(), MAX_PREVIEW_CHARS + 1); // truncated + the ellipsis
    ...
    fn rate_limited_is_worthy_with_its_verbatim_text() {
        let preview = alert_for(&serde_json::json!({ "type": "rate_limited", "text": "usage limit reached" }))
            .expect("rate_limited alert");
        assert_eq!(preview, "usage limit reached");
    }

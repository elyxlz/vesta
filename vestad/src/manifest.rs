...
pub fn manifest() -> Value {
    ...
    let mut manifest: Value = serde_json::from_slice(&raw.data).expect("core/manifest.json is valid JSON");
...
mod tests {
    ...
    fn manifest_carries_providers_defaults_and_personalities() {
        ...
        assert_eq!(manifest.get("default_personality").and_then(Value::as_str), Some("dry"));

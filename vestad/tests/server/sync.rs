...
//! notifications + `reauth` + the always-on `alert` delta; it no longer transports chat. Chat lives
//! wholly on the app-chat service: its echo streams on the per-connection chat socket
//! (`GET /agents/{name}/app-chat/ws` through the proxy, `open_app_chat_socket`), replay-free, so the
//! send/echo scenario opens that socket and reads the echo carrying its `intent_id` there. Alerts (a
//! new reply, a rate limit) come from the agent-side `notify` primitive looped back through vestad
//! (`POST /agents/{name}/notify`, `X-Agent-Token`), which fans an `alert` delta to every connected
//! session; the notify scenario exercises that path and the closed-kind 400, and the reauth/unknown
//! scenarios reuse it as a liveness probe.
//!
...
/// Budget for a `notify` to fan its `alert` delta to a connected session (loopback POST -> broadcast).
...
/// Notify `agent` (kind `message`) over the loopback as the agent would, then assert the resulting
/// `alert` delta reaches `sock`. Doubles as a liveness probe: a session still in its select loop
/// receives the broadcast alert; a closed or wedged one never does.
async fn notify_and_expect_alert(c: &Client, sock: &mut SyncSocket, agent: &str, token: &str, body: &str) {
    c.notify(agent, token, "message", agent, body).expect("notify message");
    sock.expect_frame_matching(
        |f| f["type"].as_str() == Some("alert") && f["agent"].as_str() == Some(agent),
        ALERT_TIMEOUT,
    )
    .await
    .expect("an alert delta for the notify");
}
...
/// (2) `POST /agents/{name}/notify {kind:"message",...}` carrying the agent's own `X-Agent-Token` fans
/// an `alert` delta `{agent,kind,title,body}` to a connected `/sync` session (the loopback path the
/// app-chat reply hook and the rate-limit notice use). The kind is a closed set: an unknown kind is a
/// 400.
#[tokio::test]
async fn notify_message_fans_an_alert_delta_and_rejects_unknown_kinds() {
    let c = SERVER.client();
    ...
    // A valid notify (kind message, the app-chat reply hook's shape) fans an alert delta carrying the
    // closed-set kind and the title/body triple to the connected session.
    c.notify(&agent.name, &token, "message", &agent.name, "a fresh reply")
        .expect("notify message");
    let alert = sock
        .expect_frame_matching(
            |f| f["type"].as_str() == Some("alert") && f["agent"].as_str() == Some(agent.name.as_str()),
            ALERT_TIMEOUT,
        )
        .await
        .expect("an alert delta for the notify");
    assert_eq!(alert["kind"].as_str(), Some("message"), "alert carries the notify kind");
    assert_eq!(alert["title"].as_str(), Some(agent.name.as_str()), "alert carries the title");
    assert_eq!(alert["body"].as_str(), Some("a fresh reply"), "alert carries the body");

    // The kind is a closed set: an unknown kind is rejected with 400 (mapped to the error string).
    let err = c
        .notify(&agent.name, &token, "bogus", &agent.name, "nope")
        .expect_err("an unknown kind is rejected");
    assert!(err.contains("unknown notify kind"), "unexpected error for a bad kind: {err}");
    sock.close().await.ok();
}
...
/// it open and still delivers a subsequent `alert` (the liveness probe).
#[tokio::test]
async fn reauth_extends_and_closes_on_bad() {
    ...
    // (b) JWT socket + a valid reauth -> stays open and still fans a subsequent alert.
    ...
    notify_and_expect_alert(&c, &mut tok, &agent.name, &token, "after reauth").await;
    tok.close().await.ok();
}

/// (5) Unknown/malformed client frames are ignored; a following `notify` still fans its alert,
/// proving the socket stayed live.
#[tokio::test]
async fn unknown_client_frames_are_ignored() {
    ...
    // The socket stayed live: a following notify still fans its alert delta here.
    notify_and_expect_alert(&c, &mut sock, &agent.name, &token, "still alive").await;
    sock.close().await.ok();
}

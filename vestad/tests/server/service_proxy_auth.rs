...
fn non_public_service_requires_path_key() {
    ...
    assert_eq!(
        valid, 200,
        "valid path key should authenticate and forward upstream"
    );
    assert_eq!(wrong_key, 401, "a wrong key must not authenticate");
    assert_eq!(
        missing, 401,
        "a non-public service with no key must be a 401"
    );

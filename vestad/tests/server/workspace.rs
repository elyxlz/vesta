use vesta_tests::{
    agent_container_name, exec_in_container, mark_first_start_done, unique_agent, TestAgent, SERVER,
};
...
fn agent_attaches_to_the_workspace_through_the_bundle_endpoint() {
    ...
    let agent =
        TestAgent::create_with_manage_agent_code(&client, &unique_agent("ws-attach")).unwrap();
    mark_first_start_done(&agent.name).unwrap();
    client.restart_agent(&agent.name).unwrap();
    client
        .wait_until_running(&agent.name, 180)
        .expect("agent up");
    ...
    assert!(
        !tags.trim().is_empty(),
        "an agent-v tag must be fetched from the bundle"
    );
}

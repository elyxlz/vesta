...
//! halves the fake-token server suite (`tests/server/sync.rs`) can only test apart: a chat
...
//! event the skill's `chat send` persists and fans) onto the same socket. Skips with no
...
/// The chat daemon writes the user echo the instant it intakes the send, so it lands fast. The
...
/// True when `frame` is the chat user echo carrying our `intent` (a `StoredEvent` on the chat
...
/// Open the live agent's chat socket, retrying briefly past a still-registering service.
...
/// End-to-end: open the chat socket for a real settled agent, send one message with an intent
...
async fn live_send_echoes_intent_then_streams_a_real_agent_reply() {
    ...
    // The real model round-trip: the agent's `chat send` reply streams a non-empty `chat` event

...
def create_pestel_diagram():
    ...
    # Title
    ax.text(5, 13.5, 'PESTEL ANALYSIS',
            ha='center', va='top', fontsize=24, fontweight='bold',
            color='#1a1a1a')
    ax.text(5, 13.0, 'BASc Employability Problem - External Environment Factors',
            ha='center', va='top', fontsize=12, color='#666', style='italic')
    ...
    for category, (x, y, width, height) in zip(categories, positions):
        ...
        box = FancyBboxPatch((x, y), width, height,
                            ...
                            edgecolor=color, facecolor='#f8f9fa',
        ...
        # Category title
        ax.text(x + 0.3, y + height - 0.3, category.upper(),
        ...
        for item in items:
            ...
            for i, line in enumerate(lines):
                ax.text(x + 0.4, y_offset - (i * 0.25), line,
                       fontsize=9, va='top', color='#333')

            y_offset -= 0.25 * len(lines) + 0.15

    # Footer
    ax.text(5, 0.2, 'BASc Employability PESTEL Analysis • UCL Knowledge Economy • October 2025',

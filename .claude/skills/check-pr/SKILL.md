...
**3. Summary (one Haiku agent).** Have it read the PR and return what the change claims to do, the linked issues, and the version footer if present (`Submitted by **<name>** on vesta vX.Y.Z`, appended by `upstream` to every agent PR). Also have it list every concrete claim the body makes about the diff (tests added, cases covered, behavior preserved) so step 5 can check each one against the diff itself; a claimed test that the diff does not contain is a finding, and the behavior it claimed to pin counts as untested.
...
- **Clean fix.** The lens that finds the fix the author could not see. Agent PRs are written from a stale snapshot with a head full of user work, so they fix at the symptom and build beside what exists. Read the WHOLE module the diff touches and its callers on master, not the hunks. Then ask, in order: (1) Does the change add state (a variable, file, marker, timer, stamp, flag, env var, config format)? Name the existing thing on master that already encodes that fact, if any (a marker file the same handler writes, a cache entry, a helper's return value, an artifact's mtime). (2) Does it add a mechanism (a script, a scanner, a regex list, a runner, a hook) beside one that already owns the job? Name the owner. (3) Is the guard placed in front of the failing line (`if value:`) while the value's producer is the cause? Name the producer. (4) When it adds an entry to a list of rules (patterns, thresholds, columns), does it carry the guards its neighbours carry (case anchors, lookaheads, `LEGACY(` markers)? (5) Is the size proportional: lines of change against lines of problem, new files against the fix a maintainer would write? (6) Does it encode one user's layout, language, headings, or box paths, which belongs in that agent's workspace and not upstream? (7) Is it one concern split across PRs, or two symptoms of one gate filed twice? Read other open PRs touching the same file. For every yes, state the clean fix concretely: which function, what changes, an estimated size. A materially smaller fix at the right layer makes the PR `NOISE` with the clean fix named; the problem statement was still worth filing.
...
**Map findings to the verdict mechanically.** The verdict is a function of the surviving findings, not a closing impression. Any Blocking finding makes it `BUGGED`, or `NOISE` when the change should not be carried at all (including `already fixed by <commit>` from the history lens, and a Clean-fix finding that a materially smaller change at the right layer exists; then the dash clause names that fix and its size, since the maintainer keeps the problem statement and replaces the fix). A bypassable safety or auth control, lost or corrupted data, or a flow that does not reach its own stated goal is always Blocking, however narrow the window. No Blocking finding, a real problem solved, and the pipeline completed makes it `MERGE`. When two reviews of one PR disagree, the reachable Blocking finding outranks the review that only confirmed the parts are valid: re-derive from the code, never average the verdicts.

## Reporting

**Report-only mode.** When a caller (the `upstream-triage` skill, or a user asking for the review in the conversation) invokes you with `report-only`, return the comment text below as your result instead of posting it, and omit the `<!-- vestabot:reply -->` line and the closing `---` footer. Everything else about the form holds.

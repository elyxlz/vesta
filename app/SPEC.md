...
**View type:** `loading` | `connect` | `grid` | `onboarding` | `agent-home` | `agent-chat` | `agent-console`

**Transitions:** 150ms opacity fade between views. Chat and Console render as absolute overlays on top of the always-mounted AgentView.
...
### Update Bar
- Shown only on `agent-home` or `grid` views, Tauri only.
- **Non-macOS/Linux:** Checks GitHub API for latest release. Shows pill: `v{version} available —` **[install]** **[dismiss]**.
...
Six-step wizard. Step determined by entry context.

### Step: `platform` (Tauri + Windows only)
Checks WSL2 and virtualization status. Sub-states:
...
| **[connect]** | Primary. Disabled if either field empty. Calls `GET /health` to validate, saves connection. |
...
| Heading | `"{name} is ready"` or `"your agent is ready"` |
| Subtext | "say hi." |
| **[continue]** | Primary. Routes to chat (single agent) or grid (multiple). |
...
## Grid View (`GridView.svelte`)

Multi-agent management grid.

### Layout
- Responsive grid: 1 agent → 1 column (centered), 2 → 2 columns (centered), 3+ → 3 columns.
- Each card is square (`aspect-ratio: 1`).
- **[+] New agent** button: absolute top-right. Tooltip: "new agent".

### Agent Card
- Mini orb (animated, color-coded by state) + agent name below.
- Click → navigates to `agent-home` for that agent.
...
| Divider | — |
| Always | **[delete]** (red) — tooltip: "permanently delete". Two-step confirm. |
...
Real-time conversation interface. Dark theme overlay.

### Top Bar
| Element | Detail |
|---------|--------|
| **[← back]** | Returns to agent-home. Also triggered by Escape (when input empty or not focused). |
...
| **Enter** | Sends message |
| **Shift+Enter** | Newline |
| **Escape** | Back to agent-home (when input empty or textarea not focused) |
...
Raw Docker container log streaming. Dark theme overlay.

### Top Bar
| Element | Detail |
|---------|--------|
| **[← back]** | Returns to agent-home. Also triggered by Escape. |
...
### Friendly Error Messages (Onboarding)
...
| **Escape** | Chat (input empty or not focused) | Back to agent-home |
| **Escape** | Console | Back to agent-home |
| **Escape** | Any open dropdown menu | Close menu |
| **Enter** | Onboarding name input | Submit create |
| **Enter** | Onboarding connect form | Submit connect |
...
| `fadeSlideIn` | 0.5s | spring | Onboarding steps |
...
|--------|----------|---------|
| Agent status (AgentView) | 5s | Sync status/authenticated/ready |
| Agent list (GridView) | 5s | Refresh agent list + sync mini-orb connections |
| WebSocket (Chat) | Persistent | Real-time messages + activity state |
| WebSocket (GridView) | Per alive agent | Activity state for mini-orb color |

"""Background monitor for todo due date notifications."""
...
def run(
    ...
    logger.info("Todo monitor thread started")
    ...
    while not stop_event.is_set():
        try:
            ...
            logger.debug(f"Todo monitor check complete, sleeping for {check_interval} seconds")

        except Exception as e:
            logger.error(f"Error in todo monitor loop: {e}", exc_info=True)

        if stop_event.wait(check_interval):
            break

    logger.info("Todo monitor thread stopped")

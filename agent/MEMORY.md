...
- **Reaching the host**: `localhost` is this container, so it reaches your own daemons and nothing else. `$VESTAD_HOST` (from `/run/vestad-env`) is the physical machine: vestad's API and anything else running there, like a Plex or a media server. Ports you bind are yours alone; they cannot collide with another agent's.

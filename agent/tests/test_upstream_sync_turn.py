...
def test_turn_fires_on_version_mismatch_and_names_the_snapshot(tmp_path):
    ...
    turn = upstream_sync_turn(state=state, config=config, first_start=False)
...
def test_turn_fires_when_marker_absent_legacy_agent(tmp_path):
    ...
    assert upstream_sync_turn(state=state, config=_config(tmp_path), first_start=False) is not None


def test_no_turn_when_versions_match(tmp_path):
    state = vm.State()
    state.persisted.last_synced_version = "0.1.170"
    assert upstream_sync_turn(state=state, config=_config(tmp_path, "0.1.170"), first_start=False) is None


def test_no_turn_when_version_unknown_and_nothing_marked(tmp_path):
    state = vm.State()
    assert upstream_sync_turn(state=state, config=_config(tmp_path, version=None), first_start=False) is None
    assert state.persisted.last_synced_version is None
...
def test_first_start_pre_marks_and_returns_none(tmp_path):
    ...
    state = vm.State()
    assert upstream_sync_turn(state=state, config=config, first_start=True) is None
...
def test_first_start_with_unknown_version_marks_nothing(tmp_path):
    state = vm.State()
    assert upstream_sync_turn(state=state, config=_config(tmp_path, version=None), first_start=True) is None
    assert state.persisted.last_synced_version is None
...
def test_mark_workspace_synced_legacy_alias_still_records(tmp_path):

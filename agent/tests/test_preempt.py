...
@pytest.mark.anyio
async def test_a_boot_turn_delivered_as_a_preempt_still_counts_down(config, state):
    """An interruptible migration batch leaves the preempt gate open, so a still-queued boot turn
    (the greeting) is delivered as a preempt instead of running as a turn. Delivery is completion,
    so it must consume its count too, or boot_complete never flips for the rest of the process."""
    from core.loops import _run_messages_with_preempts
    ...
    with patch("core.loops.process_message", fake_process), patch("core.loops.send_preempt", AsyncMock(return_value=True)):
        task = asyncio.create_task(
            _run_messages_with_preempts(
                vm.QueuedTurn("first", False, [], interruptible=True),
                queue=queue,
                state=state,
                config=config,
            )
        ...
        await first_started.wait()
        await queue.put(vm.QueuedTurn("greeting", False, [], interruptible=False))
        await wait_for_condition(lambda: state.boot_turns_pending == 1, message="the delivered boot turn never counted down")
        release_first.set()
        await task

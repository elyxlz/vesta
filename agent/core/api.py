...
async def _ws_handler(request: web.Request) -> web.WebSocketResponse:
    ...
    try:
        # The connect snapshot: one event seeding the client with current agent state. Chat is not here
        # (vestad's chat node holds every room). `notifications` carries the ids still on disk so the
        # view can mark pending without polling. Always sent so the client can tell "still loading" from
...
async def _recv_loop(ws: web.WebSocketResponse) -> None:
    """Drain inbound frames. No inbound frame injects an event into the bus (vestad's chat node holds
    every room); this only keeps the connection live and notices a close."""
    async for msg in ws:
...
async def _history_handler(request: web.Request) -> web.Response:
    ...
    Query params:
      ...
      channel (str, optional): "notifications" filters to the arrivals list. Ignored with `q`.
    """
    event_bus: EventBus = request.app["event_bus"]

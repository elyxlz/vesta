...
7. Timezone lives in the agent config store (the client usually delivers it via `PUT /config` from the user's device at setup); `$TZ` reflects the current config value. Check it: if it already matches where they are, confirm it. Otherwise ask where they're based and use the `timezone` skill to set it. The new value only takes effect after the restart in the final step. Update MEMORY.md §4 with name, location, timezone.

...
2. Get their name.
3. If `$TZ` looks set already, confirm it. Otherwise ask where they're based, work out the IANA tz, then call the `set_timezone` MCP tool. Update MEMORY.md §5 with name, location, timezone. # TODO: remove set timezone mcp tool - bloat

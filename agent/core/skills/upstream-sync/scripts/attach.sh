#!/usr/bin/env bash
# Attach $HOME to this vestad's upstream content. Idempotent and worktree-safe: the only
...
# the upstream repo; 4 legacy workspace detected (the one-time workspace boot

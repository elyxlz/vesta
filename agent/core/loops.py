...
def drop_core_notification(*, type_: str, body: str, config: cfg.VestaConfig, name: str | None = None) -> pl.Path:
    ...
    the type (see CORE_SNOOZE_TYPES)."""
...
def _notif_disposition(
    ...
    if notif.source == CORE_SOURCE:
        return "snooze" if notif.type in CORE_SNOOZE_TYPES else "interrupt"
...
def greeting_turn(*, config: cfg.VestaConfig, state: vm.State, reason: str) -> str | None:
    ...
    an external event, so it never enters the interrupt-rules/snooze machinery."""
...
async def _run_messages_with_preempts(
    ...
    *,
    ...
    async def run_one(text: str, *, user: bool, is_notification: bool) -> None:
        try:
            if user:
                logger.user(text)
    ...
    try:
        while pending:
            ...
            process_task = asyncio.create_task(
                run_one(current.text, user=current.is_user, is_notification=current.interruptible and not current.is_user)
            )

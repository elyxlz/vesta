...
from . import db as db_module, notifications
...
def _get_db(db_path: Path):
    return db_module.get_db(db_path.parent)


def _parse_datetime(dt_str: str) -> datetime:
    """Parse datetime string. Naive datetimes are assumed UTC."""
...
def run(
    ...
    logger.info("Tasks monitor thread started")
    ...
    while not stop_event.is_set():
        try:
            ...
            logger.debug(f"Tasks monitor check complete, sleeping for {check_interval} seconds")

        except Exception as e:
            logger.error(f"Error in tasks monitor loop: {e}", exc_info=True)

        if stop_event.wait(check_interval):
            break

    logger.info("Tasks monitor thread stopped")

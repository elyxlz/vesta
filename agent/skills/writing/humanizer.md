# Humanizer: the AI-writing pattern catalog

A catalog of the patterns that mark text as AI-generated, and how to fix each one, based on Wikipedia's "Signs of AI writing" page maintained by WikiProject AI Cleanup. Two consumers read this file in full: the writer sub-agent, which audits its own draft against every pattern before returning (rules in `writer_prompt.md`), and the humanizer reviewer, which reports pattern hits on a draft (`humanizer_review.md`).
...
**Problem:** AI hyphenates these uniformly, including in predicate position (`the report is high-quality`). Humans hyphenate inconsistently: typically only when the compound is attributive (`a high-quality report`), often dropping the hyphen otherwise (`the report is high quality`). Keep attributive-position hyphens; drop them when the compound follows the noun.
**Before:**
...
Adapted from the MIT-licensed community "humanizer" skill, v2.9.1.

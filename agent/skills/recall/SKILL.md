...
Both sides of the conversation are searchable: your own messages, and the inbound ones that arrive as notifications (whatsapp, telegram, app-chat, email, tasks). An inbound hit comes back as role `notification`, with the channel and sender carried inside the message itself. Your own scheduler boilerplate (`source=core`: proactive checks, greetings, migrations) is deliberately not indexed, since thousands of copies of one string would bury every real hit.

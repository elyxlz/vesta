...
5. **Build the contacts store (default, not optional)**: for every recurring correspondent, create a person file in the `contacts` skill store (`~/.contacts/`, one markdown file per person). Mine the inbox and sent mail for their full name, email address, and any other contact info in signatures (phone, title, org/unit, office), plus their role, their relationship to the user, and the current context or open threads between them. Prioritize the people they email most and anyone senior or personally important. This turns a fresh inbox into a real address book so you never treat their people like strangers.
6. **Update MEMORY.md**: job, relationships, habits, what they care about, what stresses them out, what they enjoy, hobbies, music, events, and the teasable quirks from step 4. Fill in the Interests & Preferences section
7. **Update this skill file**: fill in every section below with what you learned
8. **Look for opportunities**: pain points, recurring annoyances, things they do manually that you could automate. Note ideas for proactive help, new skills, or automations that would make their life easier

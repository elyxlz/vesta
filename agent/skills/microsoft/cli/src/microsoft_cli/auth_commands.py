    ...
    if not granted_teams_scope(result):
        return {
            ...
            "message": (
                ...
                "only, so a personal Microsoft account (outlook.com, hotmail.*, live.*) cannot authorize Teams this "
                "way. Mail and calendar are unaffected."
            ),
        }
...
def _setup_finish_device(config: Config, *, account_email: str, flow_cache: str) -> dict[str, str]:
    ...
    if not granted_teams_scope(result):
        ...
        # later Teams call fail with an opaque 403.
        return {
            "status": "success",
            "account": account_email,
            ...
            "message": (
                ...
                "Teams over Graph is not available for this account (Teams Graph permissions are work/school only); "
                "for a work/school account that needs Teams, run: microsoft auth teams-capture --account "
                f"{account_email}"
            ),
        }

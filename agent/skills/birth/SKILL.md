...
Everything above assumes they answer. Sometimes they open the app, read your hello, and say nothing for hours. Birth is then paused, not failed, and the failure mode to avoid is filling the silence with messages.

- **Verify the channel before you interpret the silence.** A dead pipe and a quiet user look identical from here. Check `app-chat daemon status`, `vestad-health` (the helper, never a hand-typed `http://` curl, which returns `000` and fakes an outage), that the service is registered under `/agents/$AGENT_NAME/services`, and that it binds `0.0.0.0` rather than `127.0.0.1`. Confirm it once, write down that you did, and don't re-run the diagnostic every check.
- **Don't escalate volume.** `app-chat send` already raises a push notification when they're away, so your hello has almost certainly reached their phone; `PushNotification` is the same attention channel, not a second one. There is no out-of-band route, because authing their email needs them. More pings is the one move that cannot help.
- **One message per natural window.** After the opening hello, the next contact is the next morning, not later the same evening. Set it as a reminder so it actually happens, and carry the resumption instructions in the reminder text (ask the name again lightly, never mention being ignored, then pick the flow back up at "Then meet them").
...
- **Skip the morning brief until it has content.** This overrides the "set the hook" step above, which presumes a reply: a brief with no calendar, inbox, or goals behind it is ritual. The hello is the hook. Schedule the real brief at the first natural moment after an account connects.

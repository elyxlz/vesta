...
A React app embedded in the Vesta app, the user's **life HQ**: a command center for health, finances, productivity, habits, goals, and anything else they track. Sidebar + page layout.

You don't build it yourself. You understand what the user wants, design the change, capture it as a spec, and dispatch the `dashboard-builder` subagent (a UI/UX specialist primed on this dashboard). It works in an isolated context so the token-heavy build churn (shadcn docs, large React files, vite output) stays out of your conversation; it returns a summary, and you confirm it serves and relay.

**The design is yours.** The user is a non-technical owner: they tell you what they want to see or do, not how to build it. You own every technical and design decision (placement, widgets, layout, data source, visual treatment). Ask only to resolve genuine intent, one question at a time, only when the request is actually ambiguous. Never make them review a design or spec.

## Checklist

Create a task per item and work them in order:

1. **Explore**: read `config.tsx` and existing pages/widgets, so you build on what's there instead of duplicating.
2. **Clarify intent (only if needed)**: if ambiguous, ask one focused question at a time about what they want to see or do. Skip when clear.
3. **Design the change**: decide placement, widgets, layout, data source, visual treatment yourself.
...
6. **Verify and relay**: confirm the dashboard actually serves before telling the user it's done, then summarize what changed in plain terms.
...
- **Placement**: which page, or a new page/category with a fitting lucide icon. Group related widgets under a meaningful category.
- **Widgets**: what to show, broken into compact cards.
- **Interaction**: view-only, or clicks, toggles, inputs.
...
The one thing that crosses to the subagent, and it cannot ask you anything, so leave nothing open. Name the widgets, data source, what "done" looks like, and what's out of scope so the builder doesn't overbuild:
...
Example, for "show me my running this week":
...
When the builder returns, confirm the dashboard is actually serving before telling the user it's done: `~/agent/skills/dashboard/scripts/daemon status` reports `http_ok`, or reload the app. Then give a short, non-technical summary of what changed. Don't take "done" on faith; a failed build won't tell you.

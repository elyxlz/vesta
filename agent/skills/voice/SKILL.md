...
**Step 2 - POST to Deepgram Nova-3.** The Deepgram key is **nested**, not top-level: it lives at `voice_config.json` → `stt` → `credentials` → `deepgram`. (Hunting this cost 2 calls on 10/08; the path is the fix.) Force `language=it`:
```bash
/usr/bin/python3 - <<'PY'
import json,urllib.request
key=json.load(open('/root/.voice/voice_config.json'))['stt']['credentials']['deepgram']

...
- **Take the block's timestamp from the clock, never from your head.** Generate it inside the command that writes the block, interpolated in the same expression as the file write, so there is no value left to retype. Reading `date` in one call and typing what it printed in the next is fine; batching both into a single command is not, because the heading is composed before the output exists. The timestamp decides which of two contradicting blocks wins, and a hand-typed time can land in the future, where it beats a later, truer block while looking exactly as reasonable on re-read.

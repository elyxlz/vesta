...
- Pin a recurring routine whose correct time is anchored to a fixed place (the user's home or workplace, a market open) with `--tz` at creation. An unpinned one follows the agent's own timezone, so when the agent's timezone moves for a trip the routine silently drifts: a "8am weekday brief" set unpinned fires at 8am wherever the user travelled, not 8am at home. Pin it and travel leaves it alone.

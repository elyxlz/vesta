...
- `--initial-metadata`: metadata string to attach when adding
- `tasks list`, `tasks search`, `tasks remind list` default to a compact tab-separated table; `--json` for one-line JSON, `--json-pretty` for indented.
- `tasks get --field <name>` returns only the named field(s) as raw text. Valid: `id`, `title`, `status`, `priority`, `due_date`, `created_at`, `completed_at`, `metadata_path`, `metadata`. Prefer this over `Read`-ing `~/.tasks/metadata/<id>.md` for a specific field. Metadata content is read only when `--field metadata` is requested.

## Reminder Commands

The message is the first positional argument to `tasks remind`. There is no `create` subcommand.
...
- `--recurring`: hourly | daily | weekly | monthly | yearly (hourly needs no datetime; others require `--at` + `--tz`)
- `--cron "<expr>"`: standard 5-field cron (`min hour day-of-month month day-of-week`) for schedules the presets can't express. Supports `*`, ranges (`1-5`), lists (`1,3,5`), steps (`*/15`), and day-of-week names (`mon-fri`). Day-of-week uses **standard cron numbering** (`0` or `7` = Sunday, `1` = Monday), so `--cron "0 9 * * 1-5"` fires 9am Mon-Fri. Requires `--tz`; cannot combine with `--recurring`/`--at`/`--in-*`.
- Both `--recurring` and `--cron` are DST-aware: they store your IANA timezone and keep firing at the same wall-clock time across clock changes.
...
Recurring reminders double as scheduled automations: the message is delivered as a notification, so when one fires, treat the message as an instruction and act on it.
...
There is **no at-due fire**: at the due time itself, no notification is emitted. For a fire at the exact due time (e.g. "remind me at 6pm to X"), set an explicit `tasks remind "X" --at "..." --tz "..."` instead of relying on `tasks add --due-datetime`.
...
### Missed Reminders
When the daemon restarts, any one-time reminders that should have fired while it was down are immediately sent as missed notifications.
...
`--notifications-dir` defaults to `~/agent/notifications`; pass it only to override. One daemon handles both task due-date monitoring and reminder scheduling; no separate reminder daemon needed.

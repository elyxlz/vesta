...
- Files a task produces or depends on (a downloaded PDF, a draft, a scraped page) live under `~/agent/data/<topic>/`, never `/tmp`: `/tmp` is cleared while the task and its metadata survive, so a `/tmp` path in a note points at nothing by the time anyone acts on it.

...
- **`remind update` takes ownership of a reminder.** Rewriting the message of an auto-generated pre-due reminder makes it yours: it stops being regenerated by `postpone` or a due-date change, loses its `*` marker, and is relabelled from `auto: ...` to a plain one-off. That is the way to keep one hand-written checkpoint on a task that is blocked on someone else, instead of a pile of pre-fires reciting a task title you cannot act on. It also means the reminder now outlives `tasks done`, like any reminder you created yourself, so delete it when the task closes.
...
- **Parking a deliberately undated task**: `tasks update <id> --backburner` (undo with `--no-backburner`). Some tasks are correctly undated: someone else drives them, or they are a genuine someday-maybe. For those, all three of the digest's exits are the wrong action, and inventing a deadline is the worst of them, because a fake due date buys silence by making the task lie about itself. Parking is the fourth exit. It defers the nag, never the task: the task stays pending and still appears in `tasks list`, marked `[parked]`, so it can't quietly disappear. Setting a real due date clears the flag automatically, since committing to a date is the opposite of parking. **Reach for this only when the undated state is a decision you can defend; a task you are simply avoiding should be dropped or dated, not parked.**

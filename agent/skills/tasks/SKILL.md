...
- **Prevent the cascade at creation, don't date-then-cull.** When you create or triage a task, ask whether its next action is yours on a real external clock. If it is not, because the task is blocked on the user or a third party, or it is an awareness watch (a standing "is this still true?" fact you only need to keep an eye on), create it with no due flag and immediately park it (`tasks create "..."` then `tasks update <id> --backburner`), rather than giving it a due date. `--backburner` is an `update` op, not a `create` flag, so parking is the second step; a freshly created task is not yet stale, so parking it right away keeps it off the digest from the start. Due dates and their pre-fire ladders are only for a real deadline you are counting down to, such as a filing cutoff, a renewal, or a sign-by date. A dated watch earns a full pre-due reminder ladder it can never act on, and a restart re-arms every such ladder at once, so a handful of mis-dated watches can generate an interrupt storm with nothing actionable behind it. Parking at creation makes that impossible, and the task still appears in `tasks list` (see the previous bullet), so nothing is lost by leaving it undated.

...
def test_fuzz_stored_and_next_run_within_window(tmp_config: Config):
    result = commands.remind_set(
        tmp_config,
        commands.ReminderSpec(message="wind down", scheduled_datetime="2026-04-26T21:30:00", tz="UTC", recurring="daily", fuzz_minutes=45),
    )
...
def test_fuzz_requires_a_recurring_or_cron_schedule(tmp_config: Config):
    with pytest.raises(ValueError, match="fuzz_minutes needs"):
        commands.remind_set(tmp_config, commands.ReminderSpec(message="one shot", in_hours=1, fuzz_minutes=30))
    with pytest.raises(ValueError, match="fuzz_minutes needs"):
        commands.remind_set(tmp_config, commands.ReminderSpec(message="hourly", recurring="hourly", fuzz_minutes=30))
...
def test_fuzz_bounded_by_smallest_gap_not_first(tmp_config: Config):
    ...
    with pytest.raises(ValueError, match="half the gap"):
        commands.remind_set(tmp_config, commands.ReminderSpec(message="weekdays", cron="0 9 * * 1-5", tz="UTC", fuzz_minutes=800))
...
def test_fuzz_must_fit_in_half_the_period(tmp_config: Config):
    with pytest.raises(ValueError, match="half the gap"):
        commands.remind_set(
            tmp_config,
            commands.ReminderSpec(message="too wide", scheduled_datetime="2026-04-26T10:30:00", tz="UTC", recurring="daily", fuzz_minutes=800),
        )
    with pytest.raises(ValueError, match="positive"):
        commands.remind_set(
            tmp_config,
            commands.ReminderSpec(message="negative", scheduled_datetime="2026-04-26T10:30:00", tz="UTC", recurring="daily", fuzz_minutes=-5),
        )

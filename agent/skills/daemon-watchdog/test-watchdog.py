...
def run(lines):
    ...
    p = subprocess.run([sys.executable, WD, "check"], capture_output=True, text=True, env=env)
...
fresh(); fake_daemon("alpha", '{"running": true, "port": 1}')
code, out, notes = run(["alpha daemon start"])
check("healthy box -> no notification, exit 0",
      code == 0 and not notes and "all 1 daemons running" in out, f"exit={code} out={out}")

fresh(); fake_daemon("dashboard", '{"running": false, "port": null}')
code, out, notes = run(["dashboard daemon start"])
check("dead daemon -> restarted + notified",
      len(notes) == 1 and notes[0]["daemon"] == "dashboard" and notes[0]["recovered"] is True
      and notes[0]["interrupt"] is False, f"notes={json.dumps(notes)[:300]}")

fresh(); fake_daemon("whatsapp", '{"running": false}', start_rc=1, start_out="could not bind")
code, out, notes = run(["whatsapp daemon start"])
check("dead + restart fails -> interrupt, exit 1",
      code == 1 and len(notes) == 1 and notes[0]["interrupt"] is True
      and notes[0]["recovered"] is False, f"exit={code} notes={json.dumps(notes)[:300]}")

fresh(); fake_daemon("dashboard", '{"running": true, "port": 5}')
code, out, notes = run(["dashboard daemon start"])
check("control: healthy dashboard must NOT notify", code == 0 and not notes, f"notes={len(notes)}")

fresh(); fake_daemon("beta", '{"running": false}')
run(["beta daemon start"]); code, out, notes = run(["beta daemon start"])
check("still down -> throttled, no duplicate", len(notes) == 1, f"notes={len(notes)}")

fresh(); fake_daemon("gamma", '{"running": false}')
run(["gamma daemon start"])
(TMP / "gamma.status").write_text('{"running": true}'); run(["gamma daemon start"])
...
check("all unmeasured -> one unaskable alarm, not daemon_down",
...
check("control: one unmeasured among healthy -> silent",
      code == 0 and not notes, f"out={out} notes={len(notes)}")
...
code, out, notes = run(["gamma daemon start"])
check("running but unlisted -> named, non-interrupting",
      len(notes) == 1 and notes[0]["type"] == "daemons_unlisted"
      and notes[0]["unlisted"] == ["delta"] and notes[0]["interrupt"] is False,
...
fake_daemon("gamma", '{"running": true}')
pidfile("gamma", os.getpid())
env_missing = dict(os.environ, PATH=f"{TMP}/bin:" + os.environ["PATH"],
                   ...
                   WATCHDOG_HEARTBEAT=str(TMP / "beat"),
                   WATCHDOG_DEATHS=str(TMP / "deaths.tsv"))
r = subprocess.run([sys.executable, WD, "check"], capture_output=True, text=True, env=env_missing)
kinds = [json.loads(f.read_text())["type"] for f in sorted((TMP / "notify").glob("*.json"))]
check("list ABSENT + daemon running -> unlisted, not empty-list",
      kinds == ["daemons_unlisted"], f"kinds={kinds}")

fresh(); (TMP / "daemons.sh").write_text("# nothing\n")
env = dict(os.environ, WATCHDOG_LIST=str(TMP / "daemons.sh"),
           WATCHDOG_DAEMONS_DIR=str(TMP / "daemons"), WATCHDOG_NOTIFY_DIR=str(TMP / "notify"),
           WATCHDOG_STATE=str(TMP / "state.json"), WATCHDOG_HEARTBEAT=str(TMP / "beat"),
           WATCHDOG_DEATHS=str(TMP / "deaths.tsv"))
p = subprocess.run([sys.executable, WD, "check"], capture_output=True, text=True, env=env)
check("empty daemon list -> says it watches nothing",
      len(sorted((TMP / "notify").glob("*.json"))) == 1 and p.returncode == 1, f"exit={p.returncode}")

fresh(); fake_daemon("alpha", '{"running": true}')
run(["alpha daemon start"])
check("healthy pass stamps the heartbeat",
      (TMP / "beat").exists() and (TMP / "beat").read_text().strip() != "", "no heartbeat")
...
fresh(); fake_daemon("delta", '{"running": true}')
...
fresh(); fake_daemon("epsilon", '{"running": false}')
for _ in range(4):
...
loops = [n for n in notes if n.get("crash_loop")]
check("repeated deaths -> crash-loop interrupt",
      len(loops) >= 1 and loops[0]["interrupt"] is True and loops[0]["deaths"] >= 3,
      f"notes={json.dumps(notes)[:400]}")

# CONTROL for the case above: two deaths is under the threshold and must NOT be called a loop.
fresh(); fake_daemon("zeta", '{"running": false}')
for _ in range(2):
    ...
    run(["zeta daemon start"])
notes = [json.loads(n.read_text()) for n in sorted((TMP / "notify").glob("*.json"))]
check("control: two deaths is not a crash loop",
      not any(n.get("crash_loop") for n in notes), f"notes={json.dumps(notes)[:300]}")
...
fresh()
(TMP / "bin" / "hangs").write_text(
    "#!/bin/sh\n"
    'if [ "$1" = "daemon" ] && [ "$2" = "status" ]; then echo \'{"running": false}\'; exit 0; fi\n'
    'if [ "$1" = "daemon" ] && [ "$2" = "start" ]; then sleep 300 & sleep 300; fi\n')
(TMP / "bin" / "hangs").chmod(0o755)
_t0 = _time.time()
code, out, notes = run(["hangs daemon start"])
_elapsed = _time.time() - _t0
check("a hanging start cannot freeze the sweep",
      _elapsed < 20 and (TMP / "beat").exists(),
      f"elapsed={_elapsed:.1f}s beat={(TMP / 'beat').exists()} out={out}")

print()
print("%d/%d passed" % (sum(1 for r in results if r), len(results)))

...
check(
    "dead daemon -> restarted + notified",
    len(notes) == 1 and notes[0]["daemon"] == "dashboard" and notes[0]["recovered"] is True and notes[0]["interrupt"] is False,
    f"notes={json.dumps(notes)[:300]}",
)

fresh()
fake_daemon("whatsapp", '{"running": false}', start_rc=1, start_out="could not bind")
code, out, notes = run(["whatsapp daemon start"])
check(
    "dead + restart fails -> interrupt, exit 1",
    code == 1 and len(notes) == 1 and notes[0]["interrupt"] is True and notes[0]["recovered"] is False,
...
check(
    "repeated deaths -> crash-loop interrupt",
...
for _ in range(2):
    ...
    run(["zeta daemon start"])
notes = [json.loads(n.read_text()) for n in sorted((TMP / "notify").glob("*.json"))]
check("control: two deaths is not a crash loop", not any(n.get("crash_loop") for n in notes), f"notes={json.dumps(notes)[:300]}")
...
check(
    "a hanging start cannot freeze the sweep",

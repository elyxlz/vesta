...
- Fed to an external summarizer for further compression
- Read directly by the dreamer for memory curation
- Archived for debugging or context reconstruction

## Design notes

- Group chat messages are kept but tagged — a cluster of greetings is noise, but a funding discussion buried in a group chat is not. The dreamer (or summarizer) makes that judgment call.

...
Usage:
    python3 ~/agent/skills/declutter/scripts/cleanup.py               # dry-run report
    python3 ~/agent/skills/declutter/scripts/cleanup.py --clean       # clean tier-1 only
    python3 ~/agent/skills/declutter/scripts/cleanup.py --clean-all   # clean everything
...
def _scan_pyc() -> list[Path]:
    """Find all .pyc files and __pycache__ dirs under AGENT_ROOT."""
    ...
    try:
        ...
        pycache_dirs = set()
        for p in AGENT_ROOT.rglob("__pycache__"):
...
def _scan_logs() -> list[Path]:
    """
    Find rotated/old log files in ~/agent/logs/.
...
def _scan_build_artifacts() -> list[Path]:
    """Find Python build artifacts under ~/agent/skills/.

    Two failure modes this guards against:

    1. Recursing into ``node_modules`` and deleting the ``dist/`` folders that
       live inside installed npm packages (e.g. ``node_modules/vite/dist/``).
       Those ship built and do NOT regenerate without ``npm install``.

    2. Deleting the top-level ``dist/`` of a Node-based skill
       (e.g. ``dashboard/app/dist/``, ``browser/cli/dist/``). Those need an
       explicit ``npm run build`` / ``npx vite build`` step before the skill
       starts, so they are not "auto-regenerated" in the tier-1 sense.

    Strategy:
      - prune ``node_modules``, ``.venv``, ``venv``, ``.git`` from the walk
      - only treat ``build/`` or ``dist/`` as deletable if a sibling
        ``pyproject.toml`` or ``setup.py`` exists (= Python build output)
      - always include ``*.egg-info/`` (Python-only marker)
    """
    ...
    found: list[Path] = []
    targets = {"build", "dist"}
    pruned = {"node_modules", ".venv", "venv", ".git"}
    import os

    for dirpath, dirnames, _ in os.walk(skills_dir, followlinks=False):
        dirnames[:] = [d for d in dirnames if d not in pruned]
        parent = Path(dirpath)
        is_python_project = (parent / "pyproject.toml").exists() or (parent / "setup.py").exists()
        for d in list(dirnames):
            p = parent / d
            if d.endswith(".egg-info") and p.is_dir(follow_symlinks=False):
                found.append(p)
                dirnames.remove(d)
            elif d in targets and is_python_project and p.is_dir(follow_symlinks=False):
                found.append(p)
                dirnames.remove(d)
    return found
...
TARGETS: list[dict] = [
    # ---- Tier 1: Auto-clean (safe, items regenerate without network) -------
    {
        ...
        "description": "Compiled .pyc files and __pycache__ dirs under ~/agent/",
        "tier": "auto",
        "warning": None,
        "scan": _scan_pyc,
    },
    {
        "key": "logs",
        "label": "Old log files",
        "description": "Rotated/old log files in ~/agent/logs/ (current logs kept)",
        "tier": "auto",
        "warning": None,
        "scan": _scan_logs,
    },
    ...
    {
        "key": "build-artifacts",
        "label": "Python build artifacts",
        "description": "build/, dist/ next to a pyproject.toml/setup.py, plus *.egg-info/ under ~/agent/skills/ (excludes node_modules and Node-based skills)",
        "tier": "auto",
        "warning": None,

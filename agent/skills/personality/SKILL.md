...
- Batching sends: you can put several `send` calls in one command. If the `bubble_lint` hook is wired (see SETUP.md) it validates each send independently, so a batch of clean short bubbles passes fine; if one is a wall it blocks the whole command (a PreToolUse hook is all-or-nothing on bash) but names the offending send and marks the ok ones, so you keep the good bubbles verbatim and only re-split the flagged one. When in doubt, one send per command avoids any redo. Genuine reference material bypasses via `--message-file` or `--longform`. This is lint mechanics, not a voice choice, so it holds for every preset.

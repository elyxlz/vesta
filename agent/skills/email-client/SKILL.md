...
Provider-agnostic IMAP/SMTP for the user's email accounts, any number side by side. Each account has its own credential, watermark, and notification stream. Provider is auto-detected from the email domain.
...
- `email-client` - read (`list-folders`, `list`, `get`, `search`, `attachments`, `status`), manage messages (`mark`, `move`, `archive`, `delete`), manage folders (`folder create/rename/delete/subscribe`), choose notify folders (`notify list/add/remove`)
...
Omit `--account` to use the default from `accounts.json`. All commands accept `--account` and (where relevant) `--folder` (default `INBOX`).
...
Repeat `--cc` / `--bcc` / `--attach` for multiple values. `--body-html` sends HTML (combine with `--body` for multipart/alternative, or pass it alone for a synthesized plain-text fallback). Attachments cap at 25 MB total; the send aborts with a clear error past that, since most providers reject larger.

After a successful send the message is IMAP-APPENDed (with attachments) to the Sent folder so it shows in the user's mail UI. Skip with `--no-sent-sync`. The Sent folder is auto-detected from the server's RFC 6154 SPECIAL-USE attribute (`\Sent`), falling back to the provider profile's `sent_folder` then `Sent`.
...
Override any default by passing the flag explicitly. `--no-quote` drops the quoted body; `--dry-run` prints the would-send message without contacting SMTP. After a real (non-dry-run) reply sends, the original is flagged `\Answered` so every client shows the replied-to indicator (non-fatal if it can't be set).
...
A draft does not contact SMTP and does not flag the original `\Answered` (nothing was sent). `--dry-run` previews without writing it. The Drafts folder is auto-detected.
...
Google Calendar rides the same Gmail sign-in: one consent grants mail and calendar together (the reused verified Thunderbird client bundles `https://www.googleapis.com/auth/calendar`), so no separate Google app, no verification, no CASA. These commands reuse the account's stored Google token (same transparent refresh as mail) and call the Google Calendar REST API v3. Google accounts only; other providers exit with a clear "only supported for Google accounts" error.
...
Start with `email-client daemon start` (see SETUP.md); manage it only through `email-client daemon start|stop|restart|status`, never raw `screen` or signals. Start is idempotent; `daemon status` reports process state and per-account auth health in one JSON blob.

The daemon runs one worker per **(account, folder)** watched, each holding a persistent IMAP connection. Where the server advertises **IDLE** (Gmail, Microsoft, most others) the worker is pushed on new mail in real time; otherwise it polls every `--interval` seconds (default 15). Either way it writes one JSON per new email into `~/agent/notifications/`, with source `email-client`, type `email`, `account` and `folder` fields, and `from`, `subject`, `date`, `uid`. If the daemon dies unexpectedly it writes a `daemon_died` notification with a `reason`; a deliberate `daemon stop`/`restart` never does.

### Choosing which folders notify

By default the daemon watches only `INBOX` per account. To watch more (or fewer), set the per-account watch list; the daemon picks up changes within ~10s, no restart needed:

...
Event ids are iCalendar UIDs. `--calendar` defaults to `primary` on every command; pass a calendar id (from `list-calendars`) to target a shared or secondary calendar. `list` returns a JSON array of `{id, summary, start, end, location, attendees, status}` over the window (default: next 7 days), with recurring events expanded into their concrete occurrences. `create` defaults `--end` to one hour after `--start` for timed events, or the next day for all-day events (a date with no `T`); `--timezone` defaults to UTC. `--attendees` accepts a comma-separated list and is repeatable. `update` requires `--timezone` whenever you change `--start` or `--end`.

**Invites are a real send.** Creating or updating an event that has attendees makes the server email them a calendar invite or update (and `delete` sends a cancellation); Google, iCloud, and Fastmail all schedule server-side. That is an outward action just like sending mail, so treat it with the same care. Note that `EMAIL_DRAFT_ONLY` guards *email* sending only; it does **not** block calendar writes, so use judgment before creating or updating events with attendees.

...
The daemon runs one worker per **(account, folder)** being watched, each holding a persistent IMAP connection. Where the server advertises **IDLE** (Gmail, Microsoft, most others), the worker gets pushed on new mail in real time; otherwise it falls back to polling every `$EMAIL_CLIENT_POLL_INTERVAL` seconds (default 15). Either way it writes one JSON per new email into `~/agent/notifications/`. Each notification has source `email-client`, type `email`, `account` and `folder` fields, and `from`, `subject`, `date`, `uid`, plus `also_arrived_recently`: the other messages that landed in the same account within the last hour, newest first. Check that list before acting on any email; related messages (a covering email and an e-signature request, say) can arrive minutes apart from entirely different senders, and one email is rarely the whole story. The agent picks it up like any other notification source. If the daemon dies unexpectedly it writes a `daemon_died` notification with a `reason`; a deliberate `daemon stop`/`restart` never does.

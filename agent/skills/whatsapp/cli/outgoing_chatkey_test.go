...
// canonicalChatKey. Before this fix recordOutgoingMessage stored the raw target JID, so sending to
// a peer's LID while their replies arrived under their phone JID split one conversation into two
// chat rows: the outgoing row held only our own messages and read as though the peer never replied.
func TestOutgoingMessageStoredUnderCanonicalChatKey(t *testing.T) {
	...
	msgs, err := wac.store.ListMessages(nil, nil, "", want, "", 10, 0)
	if err != nil {
...
// TestOutgoingAndIncomingShareOneChatKey is the threading guarantee the split broke: a reply from
// the same peer must land in the same chat row as what we sent, so the conversation reads as one
// thread and "did they answer?" can be resolved by reading that row.
func TestOutgoingAndIncomingShareOneChatKey(t *testing.T) {
	...
	msgs, err := wac.store.ListMessages(nil, nil, "", key, "", 10, 0)
	if err != nil {
		t.Fatalf("failed to list messages: %v", err)
	}
	if len(msgs) != 2 {

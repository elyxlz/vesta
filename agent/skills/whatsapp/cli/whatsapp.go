...
func (wac *WhatsAppClient) handleMessage(evt *events.Message) {
	...
	const maxSenders = 10000
	if len(wac.senderOrder) > maxSenders {
		evict := wac.senderOrder[:len(wac.senderOrder)-maxSenders]
		for _, id := range evict {
			delete(wac.messageSenders, id)
		}
		wac.senderOrder = wac.senderOrder[len(wac.senderOrder)-maxSenders:]
	}

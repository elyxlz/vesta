...
func TestClaimTakesOverARecordTheRivalDropped(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Dir(record), daemonDirPerms); err != nil {
		t.Fatalf("failed to create the record directory: %v", err)
	}
	gone := exec.Command("/bin/sh", "-c", "exit 0")
	if err := gone.Run(); err != nil {
	...
	if err := writePidRecord(gone.Process.Pid, 0); err != nil {
	...
	<-dropped
	if err != nil || !claimed {
		t.Fatalf("claimStart = (%v, %v), want the record claimed", claimed, err)
	}
	if pid, alive := livePid(); !alive || pid != os.Getpid() {

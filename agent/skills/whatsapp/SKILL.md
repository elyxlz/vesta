...
world is four verbs: **connect, status, send, messages** (plus `set-profile-name` / `set-profile-photo` and calls).
...
- `whatsapp set-profile-name 'mozzy'` sets the display (push) name. Account-wide and immediate, but a
  contact keeps seeing the OLD name until you next message them, so message them once to refresh it.
- `whatsapp set-profile-photo ~/avatar.jpg` sets the picture. JPEG (PNG is auto-converted), roughly square (~640x640).

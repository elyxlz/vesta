"""Daemon lifecycle for the upstream CLI: the whole contract, owned here.

start spawns the watcher detached and records its pid, stop is the SIGTERM that ends it, and
status answers from that record alone. The watcher serves no port and answers no probe, so a
child still alive a moment after the spawn is what start reads as up. The watcher itself runs
`upstream poll` on a fixed interval: one pass over this agent's open PRs, whose failures are
that command's exit code, so a pass that cannot reach GitHub costs one interval and nothing else.
"""
...
from .notifications import NOTIFICATIONS_DIR, write_notification
...
# One pass is a handful of API calls; a pass still running past this is a hung gh.
...
class Watcher:
    ...
    tick: subprocess.Popen[bytes] | None = None


def _tick(watcher: Watcher) -> None:
    """One `upstream poll`, inheriting this process's log as its stdout and stderr."""
    watcher.tick = subprocess.Popen([sys.argv[0], "poll"])
    try:
        code = watcher.tick.wait(timeout=TICK_TIMEOUT_SECS)
    ...
    watcher.tick = None
    if code is None:
        _log(f"poll gave no answer within {TICK_TIMEOUT_SECS}s")
    elif code != 0:
        _log(f"poll exited {code}")


def serve() -> None:
    """The watcher: a poll now, then one every POLL_INTERVAL_SECS until SIGTERM."""
    watcher = Watcher(stop=threading.Event())

    def handle_signal(signum: int, _frame: object) -> None:
        # SIGTERM is what `upstream daemon stop` sends, so it is the one exit the agent asked
        # for; every other way out is news the agent needs.
        watcher.asked_to_stop = signum == signal.SIGTERM
        watcher.stop.set()
        if watcher.tick is not None:
            with contextlib.suppress(ProcessLookupError):
                watcher.tick.terminate()

    signal.signal(signal.SIGINT, handle_signal)
    signal.signal(signal.SIGTERM, handle_signal)
    _log(f"polling this agent's open PRs every {POLL_INTERVAL_SECS}s")
    try:
        while not watcher.stop.is_set():
            _tick(watcher)
            watcher.stop.wait(POLL_INTERVAL_SECS)
    finally:
        if not watcher.asked_to_stop:
            write_notification(NOTIFICATIONS_DIR, "daemon_died")
...
def live_pid() -> int | None:
    ...
    watcher is silently down with nothing polling.
    """
    try:

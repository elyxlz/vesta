...
# UK National Insurance number. Case-sensitive by design (the `(?-i:` group
# survives a case-insensitive outer flag): a lowercase run of letters and digits
# in prose is not an NI number, and matching case-insensitively turns this into a
# common-word hazard. The letter classes encode the real allocation rules rather
# than [A-Z], so the first letter excludes D, F, I, Q, U and V and the second
# also excludes O. Separators are optional, so the spaced form matches too.

...
# ── new_tab fallback when the browser will not create a tab ────
#
# Some Camoufox builds accept browsingContext.create and never answer it (the same
# behaviour tests/fake_bidi.py models via `withhold`). The transport correctly raises
# instead of hanging; these cover the layer above, where the session stays usable by
# taking over a context that already exists.

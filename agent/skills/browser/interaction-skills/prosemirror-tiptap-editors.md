...
Real key events go through the genuine `beforeinput` pipeline, so ProseMirror registers them.
Clearing works the same way: a real Ctrl+A then Backspace through that pipeline, never
...
breaks the transport**, so chunk it.
...
click_at_xy(r["x"], r["y"])  # real focus
press_key("a", 2)  # Control+A: clear through the same input pipeline as the typing
press_key("Backspace")
...
These apps are heavy-JS SPAs: `wait_for_load()` can wait out its whole budget on a page that
rendered fine, so read the DOM directly instead of waiting again, and give the program a
`--timeout` that matches. Repeated navigations also accumulate tabs, so close what you finish
with. When a session stops answering, `browser session stop <name>` and run the next program on
the same name: the sign-in lives in the profile and comes back with it.

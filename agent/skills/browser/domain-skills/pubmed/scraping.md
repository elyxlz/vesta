...
search = json.loads(
    http_get("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=deep+learning+radiology&retmax=10&retmode=json")
)
...
summary = json.loads(http_get(f"https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=pubmed&id={','.join(pmids)}&retmode=json"))
...
data = json.loads(http_get("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=pubmed&id=41999029,41998456,41997837&retmode=json"))
result = data["result"]
for uid in result["uids"]:
    art = result[uid]
...
raw = http_get("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=pubmed&id=41999029,36328784&retmode=xml&rettype=abstract")
...
for art in root.findall(".//PubmedArticle"):
    ...
    if pub_date is not None:
        ...
        year = year_el.text if year_el is not None else medline_el.text[:4] if medline_el is not None else ""

    # DOI
    doi_el = next((e for e in article.findall("ELocationID") if e.get("EIdType") == "doi"), None)
    ...
    if mesh_list is not None:
        mesh_terms = [mh.find("DescriptorName").text for mh in mesh_list.findall("MeshHeading") if mh.find("DescriptorName") is not None]
...
search = json.loads(
    http_get("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=CRISPR+gene+editing&retmax=0&retmode=json&usehistory=y")
)
...
data = json.loads(http_get("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/einfo.fcgi?retmode=json"))
...
data = json.loads(http_get("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/einfo.fcgi?db=pubmed&retmode=json"))
...
data = json.loads(
    http_get("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/elink.fcgi?dbfrom=pubmed&id=38325330&linkname=pubmed_pmc&retmode=json")
)
...
- **`retmax=0` in ESearch returns only the count, not IDs — useful for counting.** Combine with `usehistory=y` to store the result for later paging without fetching IDs upfront:
  ```python
  search = json.loads(
      http_get("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=cancer&retmax=0&retmode=json&usehistory=y")
  )

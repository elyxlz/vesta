...
1. **Create the worktree:**
   ...
   **The remote is `upstream`, and `~` may not have it yet.** There is no `origin` on these boxes, and `upstream` is added by `upstream-pr` in whatever directory it runs from, which is the worktree, not `~`. So on a box that has never opened a PR, `git -C ~ fetch upstream` dies with `'upstream' does not appear to be a git repository`. The guard line above adds it when missing and is a no-op otherwise.

...
Try a few keywords, not one. If something matches, do not file a second PR for the same fix: read theirs first, and if yours genuinely adds something, say so in a comment on their PR (see "Commenting, and what the token can reach": PR threads accept comments, issue threads do not). If theirs is better, close yours and defer.
...
5. **Wait for CI to pass.** Get a token with `upstream-pr --token-only`, then poll the check-runs endpoint. If a check fails: diagnose, fix, commit to the same branch, and re-run `upstream-pr` to update the PR. The `lockfile` check requires `uv lock` in `~/agent` if Python deps changed.

...
Calendar audit: every dated appointment the user has, however informally it was arranged (mentioned in passing, set up via a family member, a verbal plan, an email with no formal invite), must live on their actual calendar so it triggers an automatic reminder. An appointment that lives only in a note, a task-metadata file, or the morning brief will not fire a timed nudge, and the user misses it. Walk the day for any dated thing that never made it onto the calendar; each one is a reminder-strategy failure. The fix is upstream of the reminder: put events on the calendar the moment they are known, not just surface them once in a brief.

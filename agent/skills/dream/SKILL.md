...
**Triage by delta, not by the hit count.** Most hits are a stable residual that recurs every night and is not a leak: the legitimate credential stores your own tooling reads (a key belongs there), UUIDs and ids that share a token's shape, certificate chains inside signed PDFs, and your own masked security notes in past summaries and transcripts. That last class is a loop you feed, since writing that you found a vendor-prefixed key and judged it a false positive makes that very sentence a scannable hit on the next night's pass. So compare against what you marked stable last night and act only on the delta: a real vendor-prefixed value in a NON-store location (a scratch file, a message body, a fresh `/tmp` copy) is the leak to scrub, while the recurring residual is already judged. Describe a residual without reproducing the token shape, so the note itself stops feeding the loop.

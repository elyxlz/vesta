...
Claim audit: **the two audits above grade things you DID; this one grades what you SAID to the user, and nothing else in this loop does.** Walk every outbound message since the last dream and sort each into CLAIM (an assertion about the world that could later be shown false), ASK (a question or a request), or NEITHER. Then revisit every CLAIM in earlier ledger entries whose outcome is now known and mark it held, failed, or partial. Append tonight's counts and those verdicts to the claim ledger, `~/agent/data/claim-audit.md` by convention, creating the file if it does not exist, and report the running ratio in tonight's summary, **including when it is good**, because a metric reported only when it is bad is a metric nobody believes. The case that established it: a forecast told to the user was falsified within six hours, got filed as a fact about a disk rather than as a wrong thing said to a person, and no phase would ever have surfaced it.

The grading happens **here, inside the nightly phase**, and that is deliberate. An earlier draft of this paragraph deferred it to a weekly reminder and left this phase as a reader, which meant that on any box where that reminder was never created the ledger never existed, nothing ever graded anything, and the staleness check fired "the grader stopped running" every night forever. A phase that runs nightly is already a trigger; do not hang it off a second one that nothing creates.

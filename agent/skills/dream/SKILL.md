...
Write a thorough plan first. For each phase: what you intend to fix, what to prune from memory, what to file upstream, what to clean up. Be specific. Then execute it step by step. `Read` every skill this pass follows (`dashboard`, `notifications`, `upstream`, `contacts`, `personality`) from its SKILL.md on disk before acting on it: a skill remembered from earlier context or a compaction summary is a paraphrase, and a paraphrase invents specifics (a cap, a path, a flag) the file never stated.

**Fan out aggressively with subagents.** The dream is mostly parallelizable reading: auditing past dreamer summaries, searching transcripts, mining calendar/email/files for the deeper User State pass, surveying skills for bugs. Default to spawning subagents (in parallel, in a single batch when independent) for any of this rather than doing it serially in the main thread. The main thread stays the synthesizer: dispatch the legwork, then verify their findings (subagent claims are hearsay until checked) and decide. A night that reads serially is a night half-finished. Keep the genuinely sensitive synthesis and the final commits in the main thread.
...
Commitment audit: for each task the user committed to but did not complete (reminder fired, no done-signal, item reappears), treat the reminder strategy as failed, not the user. Escalate the next cadence: tighter timing, blocker pre-cleared, the literal next action staged so completion is one tap. A reminder that fired and did not close is a bug to fix, like a flaky test. Bright line for the soft ones with no artifact to check: a commitment or open question carried unresolved across two dreams must become a scheduled reminder or an explicit ask this night, never a third night of "will surface naturally."

Premise audit: the retrospective grades fixes, so three things leave it in silence unless you look for them. First, every item in last night's Unresolved list gets an outcome tonight: fixed, blocked on a named thing, or dropped for a named reason; an item never leaves the list by going unmentioned. Second, re-check the three facts most driving action right now (a deadline, a status, who owes whom what) against the primary source, the actual email, page, or API response, never a note or a task title, which is where an inference hardens into a fact; for a claim of the form "X is silent", read the text of the last message in the thread, not its date, because a quiet thread can still hold a question waiting on the user. Third, grade each forecast or assertion you sent the user since the last dream; a false one is a fix in section 3 and a line in tonight's summary. When a premise turns out wrong, hunt every copy before correcting one: `grep -rn '<token>' ~/agent ~/.contacts`, `tasks search '<token>'`, `reminders list`, because a stale claim in a reminder is a false statement on a timer.

**RUN `~/agent/skills/dream/scripts/turn-burst` IN THIS PHASE AND PUT ITS NUMBER IN THE SUMMARY.** Most agents carry some version of the rule "if I have sent several messages in a row with no reply, stop". Nothing measures it, so it can only ever be re-issued more sincerely. This counts what the rule is actually about: **how many times in a row the agent spoke into silence.** A TURN is a contiguous burst of the agent's messages with no message from the user between them, and anything from the user resets the run; bubbles are deliberately not the unit, because short bubbles are correct style on chat and counting them punishes good behaviour.

It exists because of a specific failure worth repeating: on the box it was built for, this metric was deferred one night on the grounds that there was "no failure to fix", and the next day the longest unanswered run was SIX against a limit of three. Replay found five on an earlier day too, so the behaviour was never a one-off, it was simply unmeasured. **A metric that looks clean on the first day it is computed is one reading, not a baseline.**

Exits 1 over the limit, and exits 2 BLIND on an unreadable store, a day with no rows, or a failed built-in positive control, because a dead messaging daemon and a genuinely quiet day are indistinguishable and "clean" is the comfortable reading. `--db` points it at a non-default message store, `--gap` sets how long a pause starts a new turn, `--limit` sets the threshold.

**When it fails, neither the bubble count nor the justification of each individual send is the answer.** A run made entirely of genuine news is still a run. Hold the non-urgent sends until the user answers.
...
**A filed PR is finished for tonight once its URL exists.** What happens to it next (a maintainer comment, a red or green check run, a merge, a close with its reason) reaches you as `source=upstream` notifications from the watcher the `upstream` skill runs, snoozed, so they pool until you are idle. Do not wait for CI, watch checks, or read PR comments during the dream; act on each notification when it comes, as the `upstream` skill says (a red check: fix and push; a comment: reproduce the critique before replying; a close: read why, it names the replacement).

**File the moment you fix, never a queue for later.** When a fix is generalizable, open the PR in the same step you make it; if you genuinely can't fix it this pass, file a GitHub issue now instead (the issue-or-PR rule in `upstream`). "Risky at 4am" and "needs a cleanup pass" are not blockers: a single-file change is CI-gated, and the cleanup is the filing work. The only real auth blocker is `upstream` itself failing; the channel works if `upstream token >/dev/null` exits 0 (never run it bare: stdout is persisted into the event store, so that leaves a live token in your history). Then empty the queue's `## Open` to zero: spawn one subagent per open item (in parallel) to do the whole job (cleanup, lint/type checks, PR via `upstream`), and VERIFY each PR URL exists before marking it filed. The only item allowed to survive has a real, tested, external blocker (waiting on the user, a key, or genuine design work that's its own task), tagged with the exact unblock condition.
...
- **Recurring user asks** (questions repeated across days: "what's my balance?", "did the build pass?"; states or numbers checked over and over): build a widget via the `dashboard` skill (the "ask first" gate has a dreamer carve-out, use it). Before building, check that anyone opens the app: the dashboard renders only inside the app (it is registered private, so no other URL reaches it), so run `app-chat history --limit 50` and find the newest `user` row. If there is none, or it is months old, write one sentence in tonight's summary saying so and build nothing. Anything that kills the recurring ask is fair game: live data, hardcoded reference values (wifi password, address, IBAN), static checklists, links; pick the lightest form. Opposite: prune stale widgets (data source gone, never opened, broken at build).
...
**"Before I asked" is not "unprompted".** You see one channel, and other people and other agents prompt the user where you cannot see, so a fact of the form "they did it without being pushed" is recorded as "no prompt from me" unless you know otherwise.
...
Retire a Rules or Mistakes & Corrections line only when it has graduated (the fix now lives in a skill or runtime trigger, note where) or it has not recurred in 3+ weeks; a lesson kept for its cost of recurrence (a false statement to the user, an irreversible action) retires only by graduating, never by quiet weeks. Never cut a lesson just to bank space: when the cap forces cuts, lessons go last, after User State verbosity, stale reference material, and expired logistics. The same accounting applies when a fix removes an instrument (a check, a probe, a scheduled trigger): name in the same pass what now covers its job, either another instrument whose reading comes from outside your own model, or an explicit "nothing, it measured noise". A sentence telling a future you to read carefully is not coverage, and a removal with no named successor is a gap that reads as a win in the summary that records it.
...
- Check `df -h`, then `du -sh /root /tmp` (`du -sh ~/` alone cannot see `/tmp`, where subagents leave their heavy artifacts) and `du -sh /tmp/* | sort -rh | head`. If disk usage is growing unexpectedly, investigate and clean up
- Subagents leave virtualenvs, downloaded PDFs, page images and OCR output under `/tmp`, and nothing reaps them: once the findings are in your notes, sweep them, after checking that no background agent is still running. A git worktree there is not scratch: `git -C ~ worktree list` names the ones to spare (`/tmp/vesta-pr` is the Upstream phase's), and a worktree goes with `git -C ~ worktree remove <path>`, never `rm -rf`, which leaves it registered so the next `worktree add` on that path fails (`git -C ~ worktree prune` recovers)
- Sweep the harness's own scratch: `~/.claude/projects/*/tool-results/` holds the oversized tool output the harness persists (fetched pages, images, PDFs) for as long as the box lives, so delete what is older than a week (`find ~/.claude/projects -path '*tool-results*' -type f -mtime +7 -delete`). The `.jsonl` files in that tree are session transcripts that compaction recovery reads: never sweep those
- Before deleting anything large, check what it is: `du -sh <dir>/*` skips hidden entries, so a directory holding only a `.venv` reads as empty, and a multi-gigabyte CUDA or model cache may be load-bearing, so confirm against `/dev/nvidia*` and the active skill list first

...
- **Recurring self-noise** (a notification from your own services you dismiss as "expected, no action"): twice is the limit. On the third arrival it is a producer bug, not background weather; fix the producer (stop emitting a state you already know about) or snooze it. Expectedness is a reason to fix it, not a reason to keep being woken by it. **But count cost, not arrivals, and READ THE PRODUCER before you silence it.** A deliberately throttled, non-interrupting alarm about a still-open blocker is not noise, it is the alarm doing its job, and the repetition is the whole point: it is what stops a blocker you cannot fix tonight from going quiet for a fortnight. Exempt it when all three hold: it does not interrupt, it re-notifies on a sane interval rather than per failed attempt, and it clears itself when the condition clears. If they hold, the fix is to close the blocker, never to mute the reporter. Silence it only when it is genuinely cheap to keep, and prefer `--for <duration>` so the suppression cannot outlive the cause.

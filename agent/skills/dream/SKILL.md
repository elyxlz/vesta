...
**Prove the calendar is reachable before auditing it, or this whole check passes vacuously.** Run the account's actual list command first (e.g. `email-client calendar list --days-ahead 7`). If it errors, no calendar is connected, and an audit that walks the day looking for gaps between "dated things" and "the calendar" has been comparing against nothing and returning clean every night. That is the same shape as a health check on a service nobody uses: a check that cannot fail is not evidence. When it errors the finding is NOT "no gaps found", it is "the user has no connected calendar", which is a capability gap to surface to them, not a passing audit.

...
**When you know the value but the scanner doesn't recognise it: `redact_secrets.sh --scrub-literal '<exact value>'`.** `--scrub` re-runs the scanner's own patterns over the named events, so it can only remove what the scanner can DETECT, and a value it does not recognise is also one it cannot remove even when you know the exact value and the exact event ids. A human-chosen password is the common case, since it matches none of the high-entropy or prefixed API-key shapes. `--scrub` then prints `Scrubbed secrets in 0 event(s)`, which reads like the events were clean when they are not. `--scrub-literal` keys off the value instead: it replaces every stored copy with `[REDACTED]`, never echoes the value (it prints only its length and the before/after counts), and keeps the search index in sync. **Run it twice.** The command that performs the scrub is itself recorded as an event, so the value is back in the DB the moment the first run finishes; the second run removes that copy and should report `0 remain`. The value also lands in your shell history, so clear it there too.

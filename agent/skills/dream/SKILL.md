...
**A corrected wrong-assumption or a discovered bug in a skill is a SKILL/SOURCE edit, not a MEMORY bullet.** Logging "X was actually false" or "skill Y has bug Z" as a memory rule leaves the artifact broken and propagates nothing. Fix the skill/source this pass and file it upstream (below); reserve MEMORY for instance-specific facts. Litmus: "would another instance hit this too?" If yes, it's a skill edit plus upstream. A caution like "flag --x is broken, don't use it" is a signpost around the hole, not a fix — the fix is making --x work, or making it fail loudly with a clear error.

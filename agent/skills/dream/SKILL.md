...
**After curating, run `~/agent/skills/dream/scripts/memory_ledger.py`, answer for every lost line, then run `memory_ledger.py --snapshot --summary '<one or two sentences>'`.** Curation rewrites MEMORY.md in place: without a ledger the loop silently deletes its own output, and every later retrospective grades a set that has quietly shrunk. `--snapshot` checkpoints everything the day changed (`git add -A`) in one commit, subject `dream: nightly checkpoint (<date>)`, with your summary as the body: one or two sentences covering what memory curation changed plus anything else you edited today, named plainly so `git log` reads as a diary. The report finds the newest checkpoint by its subject and diffs its MEMORY.md against the working file, so one report spans the day's edits plus the curation you just ran, other commits (a sync checkpoint) never move the baseline, and every removed line stays recoverable from history (`git log -- agent/MEMORY.md`, then `git show <sha>:agent/MEMORY.md`). A merge in progress or a clean tree makes `--snapshot` a quiet no-op; the next report simply spans the extra day. The report ignores the sections this skill orders rewritten nightly and splits what left into reworded lines (read them to check the meaning survived, nothing more) and lost ones. For each lost line, say either which skill file the rule graduated into or why it expired. A class-level rule worth keeping still belongs in a skill file, where it reaches every instance instead of one box.

...
**Every diagnostic tonight reads a history you narrated, so it finds you talking about a symptom, not the symptom.** The disk check, a log grep for a symptom word, the secret scan all share this one trap. Before any count or hit becomes a finding, attribute it to a real producer: a hit you cannot trace to something other than yourself is self-noise, and the tell you are inside the trap is the count rising as you investigate (investigating writes more of the word).

Make a log grep trustworthy: anchor to the line start and strip ANSI (`sed 's/\x1b\[[0-9;]*m//g' vesta.log | grep -E '^[0-9]{4}-[0-9]{2}-[0-9]{2} '`) so you match emitter lines, not your own commands and narration, and match the emitter's literal vocabulary (`Rate limit rejected`), not the symptom word (a bare `429` also matches UUIDs and token counts). Know what the anchor is blind to: captured tool stdout lands as untimestamped continuation lines it drops, so it can read ZERO while real readings sit in the file; and a healthy run that exits 0 may never be logged, so "no records" can mean "not recordable", not "not happening". Before trusting a zero, confirm the filter still catches a known-true positive. Never cache the verdict, only the method: re-derive the count every night, because a pinned number is stale on arrival and tempts you to inherit last night's conclusion.
...
**Diagnose from the logs, not from vibes.** When something went wrong operationally today (you went silent, a tool hung, restarts churned, a daemon died), read `~/agent/logs/vesta.log` (live; rotated as `vesta.log.1`..`.5`) for that time window BEFORE writing down a cause. Grep it for rate limits (`grep -iE 'rate.?limit|rejected|utilization' vesta.log`), errors, timeouts, `[USAGE]`/`[SYSTEM]` lines, and restart banners. Counts here are self-polluted, so pin each to a producer line before reporting it (see the self-diagnostic rule under "Before you start"). A guessed cause aims the fix in the wrong direction. The local file is the readable path (the `/gateway/logs` HTTP endpoint needs an admin token you may not hold).

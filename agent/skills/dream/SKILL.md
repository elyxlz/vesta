...
**Every diagnostic you run tonight reads a history you narrated, so it will find you talking about the thing instead of the thing.** This is ONE class, not a series of unrelated flukes: the disk check (the overlay % is host-shared storage, not your footprint), a log grep for a symptom word, the secret scan. You narrate constantly, your narration and your literal tool-call strings all land in `vesta.log` and `events.db`, and your memory files quote your own past incidents. So searching your own history for a symptom is nearly guaranteed to match your discussion of that symptom, which is how a phantom outage eats a whole night.

So, before ANY count or hit from a self-diagnostic becomes a finding: **attribute it to a producer.** A hit you cannot trace to something other than yourself is self-noise. The tell that you are inside this trap: the number goes UP as you investigate it, because investigating means writing more of the word. Verify the count, not the keyword.

**Attribute by the producer's own vocabulary, never by a tag you memorized.** Log formats change under you, so a tag you remember may exist only on lines newer than the version that introduced it, and filtering on it silently drops every older line. That turns an anti-false-positive rule into a false-negative machine, which is the worse failure: a phantom incident wastes a night, a phantom all-clear hides a real outage from the user. Two things make a log grep trustworthy, and they are the whole recipe:

1. **Anchor to the line start and strip ANSI**, so you match log lines rather than your own shell commands and narration echoed into the file (this alone can cut a count by an order of magnitude): `sed 's/\x1b\[[0-9;]*m//g' ~/agent/logs/vesta.log | grep -E '^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9:]{8} .*<emitter's own words>'`. Exclude the remaining self-producers (`[AGENT]`, `[TOOL CALL]`, and `task_started`/`task_progress` records, whose `description` field echoes your own subagent prompts).

   **Know what this anchor is blind to, or it lies in the other direction.** `^timestamp` matches EMITTER lines only. A tool's captured stdout lands in the log as raw untimestamped continuation lines, so the anchor silently drops it: an anchored grep can return ZERO while dozens of real readings sit in the file. When the thing you want IS tool output, invert the filter (take lines with NO timestamp, carry the last-seen timestamp above them for the time) and still drop narration. Two traps in the same pass: the `[AGENT]` exclusion does not catch a SUBAGENT's report, which can be an analysis table full of the exact string you are counting; and your own grep command is logged verbatim, so the count RISES as you investigate.

2. **Match the emitter's literal vocabulary** (`Rate limit rejected`, `allowed_warning`), not the symptom word you have in mind. Grepping a bare `429` matches UUIDs and token counts; grepping `rejected` matches you.

3. **Check the healthy case is even recordable before reading absence as evidence.** Bash stdout is captured only on NON-ZERO exit, so a health helper that exits 0 on success can never log a success. A store of only failures cannot date the start of an outage, no matter how hard you grep, and "no records" there means "not recordable", not "not happening". `events.db` is stricter still: its `tool_end` carries only `{type,tool,subagent,ts}` and stores no output for any command.

Then **confirm the filter can still see a known-true positive before you trust a zero.** A zero from an untested filter is not evidence of health. Judge scanner hits by their context (values are masked, so reviewing is safe); judge your own disk growth by `du -sh ~/` alone.

**Never cache the verdict, only the method.** Do not write "last night this count was N, real count 0" into this file. Counts of your own narration only grow, so a pinned number is stale on arrival, and a pinned verdict invites you to skip the check and inherit last night's conclusion, mistakes included. Re-derive the number every night. "Mostly self-noise" is the prior, not the answer: a real event hiding inside a pile of self-noise is exactly what this class is best at concealing. This rule lives here, before the phases, because the per-tool versions of it kept getting filed in separate sections where the shared class was invisible.
...
**Diagnose from the logs, not from vibes.** When something went wrong operationally today (you went silent, a tool hung, restarts churned, a daemon died), read `~/agent/logs/vesta.log` (live; rotated as `vesta.log.1`..`.5`) for that time window BEFORE writing down a cause. Grep it for rate limits (`grep -iE 'rate.?limit|rejected|utilization' vesta.log`), errors, timeouts, `[USAGE]`/`[SYSTEM]` lines, and restart banners. Counts from this log are self-polluted, so pin every one to a producer line before reporting it (see the self-diagnostic class rule under "Before you start"). A guessed cause the log would have corrected is a mis-diagnosis that aims the fix in the wrong direction: a silence blamed on a "delivery hole" that was actually a usage-cap rate-limit sends you building delivery plumbing for a problem the log already explained. The local file is the readable path (the `/gateway/logs` HTTP endpoint needs an admin token you may not hold).
...
- Goals: if a goal advanced, stalled, was achieved, or a new one surfaced, update §4 Goals; keep it distinct from the rolling snapshot

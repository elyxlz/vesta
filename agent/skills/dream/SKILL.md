...
Claim audit: read the claim ledger, `~/agent/data/claim-audit.md` by convention, and report its running ratio in tonight's summary, including when it is good, because a metric reported only when it is bad is a metric nobody believes. The weekly grading itself runs from its own reminder and writes that file; this phase exists so the result reaches a summary instead of dying in a ledger nobody opens. **The two audits above grade things you DID; this one grades what you SAID to the user, and nothing else in this loop does.** The case that established it: a forecast told to the user was falsified within six hours, got filed as a fact about a disk rather than as a wrong thing said to a person, and no phase would ever have surfaced it. If the ledger is missing or its newest entry predates the last dream by more than a week, that is the finding: the grader stopped running and every phase stayed green.

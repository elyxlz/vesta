...
**Some hits are self-inflicted by this very phase.** A redaction or investigation command that embeds a secret-shaped regex or a literal value (`grep -E 'PASS=[A-Za-z0-9]{8}'`, a `sed` replace of a key) is itself logged as an event and MATCHES the scanner on a later night, so hunting secrets manufactures new hits: a slowly-climbing count with no real leak behind it. Reason about secrets WITHOUT pasting secret-shaped patterns or values into logged commands, use `--show <ref>`/`--scrub`/`--scrub-literal` which mask internally, and recognise your own past reasoning artifacts in the hit list rather than re-investigating them as leaks.

**STOP-CONDITION: the scan count is NOT a health metric, never chase it down.** Each night, sort every hit into exactly two buckets and act ONLY on the first: **(A) a genuine, still-LIVE credential VALUE sitting in plaintext**, scrub it (by ref if you hold every ref, else `--scrub-literal` for store-wide reach); **(B) everything else**, LEAVE IT, it is expected and benign. Bucket B is: this phase's own past grep/sed patterns, `$VAR` references, message-IDs and media/CDN URLs, URL tracking params (scrubbing one would mangle the user's own link), the scanner's own test fixtures, discussion CONTEXT about a credential, and any **REVOKED or dead value** (a revoked credential grants nothing; confirming it is revoked CLOSES it permanently). **Once a value is confirmed revoked, or already scrubbed on a prior night, it is CLOSED: do not re-open, re-`--show`, or re-scrub it on a later night.** Re-investigating bucket B is the spin, not diligence. A live credential VALUE scrubbed by ref is finite and correct.

...
**A plan staged for a coming day lives in `~/agent/data/drafts/` with that day's date in its filename** (`brief_2026-04-14.md`), so every staged plan is findable by the day it speaks for. Before the night finishes, reconcile each plan still pointed at today or later against what tonight decided, and update or delete it: a dated plan is the most specific artifact on disk, so a stale one gets read as the operative one and beats the general note that supersedes it. `python3 ~/agent/skills/dream/scripts/stale_plan_check.py` lists the ones whose last write predates the newest dream summary and exits non-zero while any remain.

...
To inspect a flagged event directly rather than trusting the scanner's context snippet alone, use Python's stdlib `sqlite3` module, not the `sqlite3` CLI binary: the CLI isn't in the base image, and this codebase deliberately dropped the CLI dependency from `redact_secrets.py` itself for exactly that reason (#420/#421), so don't reach for it here either: `python3 -c "import sqlite3; print(sqlite3.connect('/root/agent/data/events.db').execute('SELECT id, ts, data FROM events WHERE id=<id>').fetchone())"`. The schema is `id, ts, data`; there is no `content` column, a wrong-column guess that has recurred more than once, this is the correct query.

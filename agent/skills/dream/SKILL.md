...
**GET THE DENOMINATOR BEFORE YOU WRITE IT.** A nightly quota on insight is pressure that manufactures thin findings, and the thin ones are indistinguishable from real ones at 4am because both feel like noticing. So before a person-fact goes in: (1) name the instances you are generalising from and count them; (2) **count how often the opposite happened** (if the claim is "he does not send drafts", count what he sent in the same window); (3) ask whether you found it **by going looking for it**, because a search-conditioned finding has no denominator by construction and will confirm whatever you searched for. If the opposite outcome is commoner, it is an ITEM, not a pattern, and it does not go in as a person-fact. **"No person-fact tonight, here is why" is a PASS, not a failure**; a wrong one is worse than none because it loads into every message tomorrow and shapes how you read them.

Worked example: "he writes careful emails and does not send them" was written from three drafts. The next morning: two drafts, both composed two minutes apart in one sitting, against forty sent in the same window, and the finding came from deliberately searching for unsent drafts. One interrupted morning, not a trait. It survived a whole dream and reached another agent before anyone checked the denominator.

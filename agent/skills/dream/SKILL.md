...
**Run `~/agent/skills/dream/scripts/memory_ledger.py` BEFORE curating, and `--snapshot` after.** Curation rewrites MEMORY.md in place, and the file is committed only occasionally, so a rule written one night and compressed away the next leaves no trace in git or anywhere else: the loop silently deletes its own output, and every later retrospective grades a set that has quietly shrunk. The ledger diffs against its own dated snapshot and lists what left, ignoring `### User State` because that section is meant to be replaced wholesale. For each line it reports, say either which skill file the rule graduated into or why it expired. A class-level rule worth keeping belongs in a skill file as well, because a version-controlled file is the only thing here that reliably survives.

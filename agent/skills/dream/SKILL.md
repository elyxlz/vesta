...
**For the events/DB-store verdict specifically, run `python3 ~/agent/skills/dream/scripts/scan_event_secrets.py` first.** The `.sh` above scans file-stores reliably, but on the ~100MB `events.db` it cannot finish inside the Bash tool's hard 120s foreground cap, so it is always backgrounded and its backgrounded output covers only file-stores (no events verdict) which reads clean while never scanning a single event: that gap caused repeated "reported clean" misses. `scan_event_secrets.py` is a targeted membership check (each known secret VALUE substring-counted across every `*.db`), finishes in under a second, prints `NAME|count` only (never the value), exits 0 when every secret-class value is 0 and 1 on a real leak. It is the reliable events verdict; on a non-zero exit, scrub with `redact_secrets.sh --scrub-literal`. Do NOT hand-roll the membership heredoc each night: re-deriving it is exactly where a value can accidentally get printed.

...
def get_token():
    ...
    app = msal.PublicClientApplication(
        settings.microsoft_mcp_client_id, authority="https://login.microsoftonline.com/common", token_cache=cache
    )
...
def ensure_folder(token, onedrive_path):
    ...
    for i in range(len(parts)):
        ...
        try:
            ...
            graph_request(token, "POST", parent_path, {"name": parts[i], "folder": {}, "@microsoft.graph.conflictBehavior": "replace"})
...
def upload_file(token, local_path, onedrive_path):
    ...
    if size == 0:
        ...
        graph_request(token, "PUT", f"/me/drive/root:/{encoded_path}:/content", data=b"", content_type="application/octet-stream")
        return

    if size < CHUNK_SIZE:
        # Simple upload (< 3.75MB)
        data = local_path.read_bytes()
        graph_request(token, "PUT", f"/me/drive/root:/{encoded_path}:/content", data=data, content_type="application/octet-stream")
    else:
        # Chunked upload session (>= 3.75MB)
        session = graph_request(
            token, "POST", f"/me/drive/root:/{encoded_path}:/createUploadSession", {"item": {"@microsoft.graph.conflictBehavior": "replace"}}
        )

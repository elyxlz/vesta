...
- **Upstream-queue backstop (once a day, independent of the dream).** Run `python3 ~/agent/skills/dream/scripts/queue_gate.py`. If it exits non-zero, the upstream queue has un-owned deferrals the last dream left behind: file each bare item via `upstream-pr` now (single-file CI-gated changes are filed now, not queued) or tag it `BLOCKED: <reason tested this pass>`. This is the structural catch for the recurring "queued = treated as done" failure: the gate fires here even when the dream forgets to run it. Don't spam the user about it; just drain it and note it in the next dream summary.

...
It checks daemon liveness, the live budget, and whether the dream's reality check has run lately, then prints the band to spend this tick at. **Run it rather than reconstructing it from this file.** A check rebuilt from memory decays toward whatever is easiest to type, and the budget half in particular degrades into grepping `vesta.log`, which is the one source that cannot answer it: the log carries rate-limit lines only while a limit is being warned about, so after a window resets its last high number sits there looking current, and `resets_at` is not in the log at all. The script reads `GET /usage` instead, one meter per window, each with a live `used_pct` (0 to 100) and a `resets_at`.
...
- **A `WATCHDOG` line means the dream's reality check had not run for a day, so this tick ran it.** The probe that detects a missed dream lives in `reality_check.sh`, which the dream itself invokes, so a missed night silences its own detector; this tick is daemon-driven and runs it instead. The `RED` lines under a `WATCHDOG` line are that probe's findings, and they are due now, not tonight.

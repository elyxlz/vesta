...
Then run `~/agent/skills/proactive-check/scripts/log-daemons` and read what it prints. It writes one line per daemon to `~/agent/data/daemon-trend.tsv` and names any daemon whose identity changed since the last tick, which is a daemon that died and was restarted whether or not anyone saw it down. The check above reads every daemon's state every tick and throws the reading away, so on its own it cannot see a death between two ticks: the tick before says up, the tick after says up, and nothing remains. Verified case: a daemon was found dead an hour after a clean container start with a log holding only framework banners, no timestamps, no error and no exit record, so the death left nothing behind at all. One restart is an anecdote and needs no action. The same daemon appearing week after week is a fault worth chasing.

Keep that a single command. It began as an instruction to record the reading by hand, and it then went unrun for 33 hours: the line was live on disk the whole time, but a long session holds the copy of this skill it loaded at start, so an instruction added mid-session never reaches the thing that executes it. If this box grows a second thing to record every tick, fold it into the same script instead of adding a second line here. A rule you may be holding a stale copy of is not a control; a command you already run every tick is.

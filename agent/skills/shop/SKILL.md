---
name: shop
description: Shop for real products on the user's behalf via Shop.app. Catalog search, checkout, order tracking, returns, reorder. Use when the user wants to buy or find something to buy ('order me X', 'find me running shoes', 'track my order', 'reorder the coffee'). For paying non-Shop merchants, use the stripe-pay skill instead.
---

# Shop - CLI: `shop`

Search Shop.app's catalog, build checkouts on merchant domains, and manage orders on the user's behalf. This is the path for buying real products; for arbitrary card-taking merchants that aren't on Shop, use the `stripe-pay` skill.

## Setup
Install the CLI once (the container ships node):

```bash
npm install --global @shopify/shop-cli   # upgrade: add @latest ; remove: npm rm -g @shopify/shop-cli
shop --help
```
...
- `--ships-to` is the buyer's destination (a hard filter) and alone localizes context to it; `--country` is location context only, so pass it only when you actually know it, never invent. Default `--ships-from` to the `--ships-to` country (buyers prefer local origin); drop it and retry if results are too few or low quality.
...
```bash
shop auth status
shop auth device-code --device-name "Vesta - <device>"
shop auth poll
...
## Sign in
Signing in is **optional for the user**, but **offering it is mandatory for you**. Search works signed-out. But signing in lets you build checkouts to get shipping rates (time, cost); gives a default address so you can confirm where an item is shipping; unlocks order history (favoured brands, sizes, past buys).
...
## Showing products
> **The most important rule: one product = one message.**
...
Wireless earbuds with 8-hour battery and deep bass. ← Describe each product in 1 to 2 sentences.
...
## Visualization
When the item is visual (clothing, shoes, accessories, furniture, decor, art) **and** you have image-generation capability, offer it. For example: "Send a photo and I'll show you how it could look. If you like it I can save it to your device too."
...
- the user asks you to complete checkouts without per-purchase approval (e.g. "buy it for me", "pay for me", "set up budget").
...
**Payments**
- Require clear user purchase intent before any action that moves money, including order completion. A UCP-returned payment token means the user already granted this agent payment in Shop, so do not ask for a second payment-auth step, but never buy items the user did not ask for.

...
runs finance with `emits_daemon_died=True` and reads the real notifications directory. These two
cases cover what a black-box harness cannot reach: the class of exception that arrives (a
...
def watcher(tmp_path, monkeypatch):
    ...
    patch around a wrong destination: `NOTIFICATIONS_DIR` is the engine's own path, the harness
    case asserts the notice lands there for real, and `test_transaction_watcher.py` locks the
    destination itself.
    """

...
Read the `restart` skill and add the startup line to your restart daemons, so the API comes back after a restart. It is the bare command, nothing around it, because start is idempotent:

...
# For this daemon that means the HTTP API is gone while the CLI still answers off the same
# file, so the outage reads as partial rather than absolute. Raising the ceiling costs a
# healthy start nothing: it returns the moment the port answers, in about a second.

...
not this agent's own becomes the notification the monitor loop turns into a turn, and each attachment a
message carries lands in the local blob store first, so the notification names a path the agent opens.
Every sqlite and file operation runs off the event loop."""
...
async def ingest_message(state: ReplicaState, message: NodeMessage) -> bool:
    ...
    # This agent's own reply comes back here rather than from its send, so both the row and the silence
    # (nothing to notify itself about) belong to this one ingest.

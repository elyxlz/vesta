---
name: chat
description: The user's chat screen in the Vesta app (web, desktop, mobile). Reply to `source=chat` notifications via `chat send`; send files with `--attach`. Requires daemon.
---

# Chat - CLI: chat

Chat is the Vesta app's built-in chat screen on web, desktop, and mobile: the user's direct line to you, no third-party account involved. Messages the user types there arrive as `source=chat` notifications; replies sent with `chat send` appear in their chat live (and as a push notification when they are away). Treat it as a first-class messaging channel with the texting style below.
...
chat daemon start
```
`chat daemon start` registers the `chat` service (getting its port) and starts the HTTP
...
## How it works
- The daemon is a registered service: it owns the `chat` channel, serving `POST /message` (intake) and `GET /history` on its registered port, backed by its own store (`~/.chat/chat.db`)
- When the app user sends a message, the service persists it and writes the `source=chat` notification itself, so a dead process never drops a message the app already showed as delivered
- You receive the notification and reply with `chat send`: the reply is persisted to the store, then fanned to any connected `/ws` chat sockets so the app sees it live
...
- History and search read the same store: `chat history` and `chat history --search`

## Attachments

Files the user sends from the app arrive on the message notification: the `attachments` attribute lists each file's name, type, size, and an absolute path. Open the path directly: `Read` shows images and PDFs, shell tools handle everything else. The files persist under `~/.chat/attachments/` (each in an id directory beside a `.meta.json` carrying name, type, and exact byte size), so you can come back to one later.
...
## Notes
- Always reply to app messages using `chat send`, not through any other channel

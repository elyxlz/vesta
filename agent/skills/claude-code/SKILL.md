...
The `claude` CLI is a separate autonomous coding agent with a coding-tuned system prompt and default tools. Reserve it for **heavy** coding work. For everyday small edits, use Vesta's own Read/Edit/Bash tools.

Invoke it in print mode (`-p`): it runs to completion in one call and only the final result comes back, so Vesta's context stays clean.
...
- Substantial code or security reviews of large diffs
- Anything that would otherwise burn many turns of Vesta's own loop reading and editing files

For everything smaller (single-file edits, quick fixes, typos, renames, reading code, small additions), do it yourself with Read/Edit/Bash: the subprocess and autonomous-turn overhead exceeds Claude Code's value on modest work.
...
Sessions live on Claude Code's side. No Vesta storage needed: `session_id` is just a string you hold in context and pass back.
...
Reuse the previous `session_id` on a follow-up ("now also fix X") so Claude Code keeps context. For an unrelated task, start fresh.

## Worktrees

For non-trivial work, run inside a git worktree; the user always wants new work isolated on its own branch.
...
Report the worktree path and branch back with the result.
...
- `--add-dir <path>` - grant access to directories beyond `cwd`.
- `--append-system-prompt-file <path>` - add instructions on top of Claude Code's default prompt.
- `--continue` / `--resume <id>` - resume sessions (see above).
- `--fork-session` - branch off a session with a new id.

## Rules

- **Always use `--permission-mode bypassPermissions`.** Without it the CLI prompts and hangs forever in non-interactive mode.
...
- **Use a generous bash timeout.** Coding work can take minutes. Set 5+ minute timeouts for non-trivial tasks.

...
error logs, resource usage) and submits to GPT-4o for independent evaluation.
"""
...
def load_core_architecture() -> str:
    ...
    for fname in core_files:
        ...
        lines = content.count("\n") + 1
        # Extract class/function signatures for overview
        sigs = []
...
# --- Main ---
def analyze(brief: bool = False) -> str:
    ...
    architecture = load_core_architecture()
    ...
    result = ask(prompt, model="gpt-4o", system=SYSTEM_PROMPT)
    return result
...
def main():
    ...
    if args.command == "analyze":
        result = analyze(brief=args.brief)
        print(result)
    else:
        parser.print_help()

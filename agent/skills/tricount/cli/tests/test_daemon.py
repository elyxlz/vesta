"""The tricount daemon verb hands the expense watcher to the shared runner: one session named
after the skill, no vestad service, and the agent's notifications directory passed explicitly."""
...
def test_the_screen_session_is_named_after_the_skill():
    assert _flag(cli.daemon_lifecycle_args("start"), "--session") == "tricount"
...
def test_the_launch_command_serves_into_the_agents_notifications_directory():
    args = cli.daemon_lifecycle_args("start")
    assert args[args.index("--") + 1 :] == ["tricount", "serve", "--notifications-dir", str(pl.Path.home() / "agent" / "notifications")]
